...
## FOURTH FOLLOW-UP (2026-08-15 — actual CI run surfaced pre-existing config bugs)

Pushed the Third Follow-up's QRGateway fix and, rather than assuming green, pulled the actual GitHub Actions results for that commit (job/step/artifact-level, via the Actions REST API — job *log* downloads 403 without admin rights on this repo, same limitation as earlier follow-ups, but check-run status, job step lists, and artifacts are readable). The "QRGateway E2E Tests" workflow was still red — not a regression in the QRGateway fix itself, but three independent, genuinely pre-existing CI/config bugs that the suite had never actually gotten far enough to hit before:

- **Firefox and WebKit couldn't launch at all.** `playwright.config.ts`'s top-level `use.launchOptions.args` applied `--use-fake-ui-for-media-stream`/`--use-fake-device-for-media-stream` (Chromium-only CLI switches) to every project. Firefox/WebKit's launchers reject unrecognized arguments outright ("Cannot parse arguments: Unknown option") and refuse to start — meaning 2 of 3 browsers in the matrix have been 100% unable to run *any* test, on *any* commit, for as long as this config has existed. Fixed by scoping those args to only the Chromium-based projects (`chromium`, `mobile-chrome`).
- **The `node-version: [18.x, 20.x]` CI matrix has an entry that can never work.** `package.json` pins `vite: ^8.1.1`, which requires Node ≥20.19; Node 18.x's `config.webServer` (`npx vite ...`) exits immediately with code 1, so that entire matrix leg was dead regardless of test content. Bumped the matrix to `[20.x, 22.x]`.
- **My own new "large image" test used a hardcoded macOS path** (`/private/tmp/turbofix-large-test.jpg`) — `ENOENT` on Linux CI runners, which don't have `/private/tmp`. Switched to `os.tmpdir()`.
- **My own new OTP-mock test needed CORS handling for a cross-origin mocked response** — Firefox enforces CORS on `route.fulfill()` responses more strictly than Chromium, so the fetch was rejected before the app ever saw the mocked body; the CORS *preflight* OPTIONS request (no JSON body, so it didn't match an `action` and fell through to the real network) needed answering directly too. Fixed by adding `Access-Control-Allow-Origin` headers and handling `OPTIONS` in the route handler. This fixed Firefox; WebKit still can't receive `route.fulfill()` cross-origin responses at all even with correct CORS handling (confirmed via a clean repro — a known Playwright/WebKit gap, not a product or logic bug). Skipped just that one test on WebKit with `test.skip(browserName === 'webkit', ...)` and a comment explaining why — OTP send/verify has real, unmocked coverage elsewhere (`tests/whatsapp-otp-edgecases.spec.js`) that does run on WebKit.
- Also fixed the "back button during phone gate" test's `about:blank` assertion to accept either outcome browsers legitimately differ on (Chromium/WebKit treat a directly-`goto()`'d page as having a prior about:blank history entry; Firefox treats `goBack()` as a no-op) — the real intent ("doesn't crash") doesn't require picking one.
...
## FIFTH FOLLOW-UP (2026-08-15 — Operational Health Score, from a competitor review)

User asked what features to borrow from a competitor CMMS (maintenance-master.com). Its headline feature — a single 0–100 "Operational Health Score" blending machine condition, PM discipline, parts readiness, and ticket pressure into one plant-level number with a month-over-month trend — turned out to be a **real, already-acknowledged gap**, not a new idea: `src/pages/Dashboard.jsx` already had a `healthScore` field (uptime/SLA/PM composite) and a `healthTrend: 0, // requires historical data; honest zero` — a deliberate placeholder for exactly this feature, just never built out. This follow-up builds it out, not from scratch but as a genuine extension of that existing, already-documented gap.

**Built:**
- `src/utils/operationalHealth.js` — pure `computeOperationalHealth()`, equally-weighted (25% each) across 4 drivers: machine health (via `machineHealth.js`'s `machineDisplayStatus`, cross-referenced against the *live* tickets list rather than a page-local, often-stale `track_record` field), PM on-time (active `pm_schedules` only — a paused schedule can't be "overdue"), parts availability (via `inventoryMetrics.js`'s existing critical/at-risk stock classification), and ticket/SLA pressure (via `ticketSla.js`'s existing breach detection). Every driver defaults to a documented 100% + honest "no data yet" detail when its input array is empty, rather than crashing or reading as a fake crisis. 13 new unit tests (`src/__tests__/operationalHealth.test.js`), all passing; full suite still 1205/1205 (up from 1192).
- `supabase/migrations/20260815000000_operational_health_snapshots.sql` — one row per company per day (`UNIQUE(company_id, snapshot_date)`), RLS-scoped via `get_current_company_id()` exactly like every other per-company table in this schema. **Not yet applied to production** — same as every other migration in this audit, needs the user's own `supabase db push`.
- `src/lib/operationalHealthSnapshot.js` — the only place that actually touches Supabase for this feature (`operationalHealth.js` itself stays pure, no I/O, matching the existing `machineHealth.js`/`ticketSla.js` convention). `persistTodaySnapshot()` upserts today's score; `fetchTrend()` compares against the closest snapshot ≥30 days back and returns `null` (not a fabricated 0) when there's nothing old enough to compare against.
- Wired into `src/pages/Dashboard.jsx`: `metrics.healthScore` now comes from the real 4-driver score instead of the old 3-signal formula; a new `useEffect` persists/fetches the trend (skipped entirely for demo/no-company sessions, so demo numbers can never pollute a real company's history) and both errors are caught and `console.error`'d, never thrown — the score still renders correctly even if the snapshot table is unreachable.
- A new "Operational Health" card in `src/components/dashboard/LimbleCmmsDashboard.jsx` (the only dashboard component actually rendered — confirmed via the Second Follow-up's dead-code findings, so this intentionally did not touch the orphaned `OwnerDashboard30s.jsx`/`MasterTabbedDashboard.jsx`, which read the same `healthScore`/`healthTrend` fields but aren't reachable from any route): score, band (Healthy/Attention needed/Critical), all 4 driver percentages with detail text, and the trend line.

**Verified live in the browser, not just unit-tested**: demo session → 49/100 CRITICAL with correct per-driver detail ("2 machines down", "3 parts short", etc.); a real empty-company session → clean 100/100 HEALTHY with honest "No machines on record" / "No parts tracked yet" detail strings, no crash; a real session with a company_id set → confirmed the snapshot save/fetch calls actually fire with the right shape, hit `Could not find the table 'public.operational_health_snapshots' in the schema cache` (since the migration isn't applied yet), and **degraded gracefully** — logged via `console.error`, dashboard still rendered the score correctly, no visible error to the user.

**What's left, and needs the user's own action**: apply the migration (`supabase db push`, or run the SQL directly) before the trend line will ever show a real number instead of "Not enough history yet for a trend" — this is a schema change to a live, shared database, so it was written and verified but deliberately not applied automatically, same as every other migration in this audit.

## SIXTH FOLLOW-UP (2026-08-15, same day — the remaining 4 competitor-review gaps)

User approved implementing all four remaining gaps from the competitor review, not just the Operational Health Score above. All four verified: full test suite went from 1192 → 1263 (71 new tests across 5 new util files, zero failures), build clean, no new lint warnings (one pre-existing dead-code warning in `Dashboard.jsx` — `slaPct`/`pmPct` left over from the old health-score formula — cleaned up in passing).

- **Vendor performance tracking** (`src/utils/vendorPerformance.js`, 10 tests) — pure rollup over data already collected: `tickets.outsource_vendor` (set by `escalation_service.mark_outsourced()`) and `machines.amc_provider`/`vendor_name`. No new schema. Surfaced as a 4th tab on `src/pages/Support.jsx` (chosen because it's the only page that already fetches both `tickets` and `machines` directly) — per-vendor tickets assigned/resolved/pending, average resolution time (reusing `ticketSla.js`'s existing `ticketAgeHours`, so the number means the same thing as the SLA board's), and cost. A vendor with AMC coverage but zero tickets still gets a row, on purpose — "we pay them but never call them" should be visible, not hidden.
- **OEE (Overall Equipment Effectiveness)** (`src/utils/oee.js`, 13 tests) — the one gap that genuinely needed new data, not just a rollup: Availability was already derivable from `tickets.downtime_minutes`, but Performance and Quality need production counts TurboFix never collected. Added: two machine config fields (`ideal_cycle_time_seconds`, `planned_minutes_per_shift` — migration `20260818000000_oee_tracking.sql`, plus the matching allow-list entries in `onboard_team_member`'s `update_machine` action and 5 call sites in `Machines.jsx` that read/write/hydrate those fields), an append-only `production_logs` table (one row per shift entry, summed per day — a plant running 3 shifts logs 3 rows, not an upsert), and a "Log production" quick-entry + OEE readout in `MachineDetailDrawer.jsx`. A machine with zero logged shifts shows Availability only, honestly labeled "not tracked yet" for Performance/Quality/OEE rather than a fabricated score — confirmed live in the browser (demo machine, no ideal cycle time configured yet) and confirmed the write path attempts a real Supabase insert and surfaces failures inside the drawer rather than silently.
- **Bulk CSV import for machines and parts** (`src/utils/machineImport.js` + `src/utils/partsImport.js`, 24 tests, one hand-rolled RFC-4180-ish parser shared between them) — the #1 friction point every CMMS comparison in the research cited for SME onboarding. Download-a-template-or-use-your-own-spreadsheet flow, client-side parse + validate + preview (invalid rows flagged with their exact line number, valid rows still import), then a real quota-aware bulk insert. Wired into `Machines.jsx` (respects the existing machine-quota gate) and `Inventory.jsx` (mirrors `addItem`'s existing "add locally, sync best-effort" pattern rather than inventing a new one). **Caught two real bugs via the tests before they shipped**: the machine template's own generated "Criticality (low/medium/high/critical)" header didn't match its own alias list (parenthetical hint broke the match, silently defaulting every imported row to "medium" regardless of what the file said), and the parts template's "Bin / location" header had the same class of bug. Both fixed; added a template-round-trips-through-its-own-parser test to `partsImport.test.js` specifically so this class of bug can't recur silently. Also caught mid-build: the CSV-import modals' error state was being set on the *page-level* error banner, which sits **behind** the modal's dark overlay — a failed import would fail silently from the user's point of view. Fixed by giving both modals their own local error state rendered inside the modal itself; verified live that a forced failure now shows correctly where the user is actually looking.
- **"How was this fixed last time?" at report time** (`src/utils/similarFixes.js`, 11 tests) — token-overlap matching (same technique as the existing `dynamicChecklist.js`, no new dependency) between the sentence being typed in `ReportBreakdown.jsx`'s issue box and past tickets' `repair_action`/`root_cause`, live as the reporter types. Same-machine matches get a score bonus, not a bypass — an unrelated past issue on the same machine is noise, not a fix. Deliberately distinct from the existing `HistorySuggestion.jsx` (shown once a machine is picked, themed on recency/category, not matched against the live sentence) — the two now cover different moments in the same flow rather than overlapping. **Found a real, separate pre-existing bug while building this**: `HistorySuggestion.jsx` reads `resolution_note`/`repair_note`, fields that exist only in the hand-written demo dataset — the actual `tickets` table's columns are `repair_action`/`root_cause` (confirmed across 5 migrations), so that component's "last repair" line has silently shown nothing for every real, non-demo ticket in production. Flagged as a separate follow-up task rather than fixed here (out of scope for this pass, and touches a different component's established behavior).

### Still pending the user's own action (schema changes to a live, shared database — none applied automatically)
- `20260813120000_drop_remaining_stale_factory_policies.sql` / `20260813130000_additive_company_id_ticket_policies.sql` (written 2026-08-13, reviewed, additive/safe-by-design — see their own header comments)
- `20260815000000_operational_health_snapshots.sql` (this session)
- `20260818000000_oee_tracking.sql` (this session)

All four are independent — order doesn't matter, `supabase db push` applies whichever are still pending together.
