# TurboFix Docs

- [PRD](./TURBOFIX_PRD.md)
- [Phase 0 Release Baseline](./PHASE_0_RELEASE_BASELINE.md)
- [Technical Architecture](./TURBOFIX_TECHNICAL_ARCHITECTURE.md)
- [Security & Access](./TURBOFIX_SECURITY_ACCESS.md)
- [Frontend Spec](./TURBOFIX_FRONTEND_SPEC.md)
- [Feature Ticket List](./TURBOFIX_FEATURE_TICKET_LIST.md)

Short version:

- analytics stays the engine
- TurboFix stays the workflow layer
- the machine stays the focus
- review and verification stay in the loop
