...
## Corpus Check
- 473 files · ~572,681 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3743 nodes · 7965 edges · 278 communities (198 shown, 80 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 658 edges (avg confidence: 0.58)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d89fe839`
...
## Communities (278 total, 80 thin omitted)

### Community 0 - "UserRepository"
Cohesion: 0.03
Nodes (94): Role, get_ai_feedback(), get_documents(), get_escalation_config(), get_machine_records(), get_parts(), get_settings(), get_shift_config() (+86 more)
...
Cohesion: 0.05
Nodes (56): create_reset_token(), decode_access_token(), decode_reset_token(), get_current_admin(), get_current_user(), hash_password(), _password_fingerprint(), Phase 5 - Document Vault authentication.  A small, real (not stubbed) JWT auth l (+48 more)

### Community 3 - "DocumentRepository"
Cohesion: 0.07
Nodes (96): RFC-4180, MachineRepository, Machines(), MACHINES, NOW, opts, PM_LOGS, TEAM (+88 more)
...
Cohesion: 0.09
Nodes (28): ask_maintenance_assistant(), _event_line(), get_machine_events(), get_root_cause_analysis(), _live_data_answer(), _machine_context(), _plant_context(), Dashboard router — per-company KPI dashboard + root cause analysis endpoints. (+20 more)

### Community 9 - "main.py"
Cohesion: 0.07
Nodes (50): EventRepository, Read/write access to the MachineEvents data entity., Append a new event row. Keys must match MACHINE_EVENTS_HEADER., Return all events for a machine, oldest first., Return all events for a company., ai_enabled(), Return True if any AI provider is configured and ready., _detect_and_store_language() (+42 more)
...
### Community 20 - "react"
Cohesion: 0.16
Nodes (19): CRITICALITY_ROWS, DashboardChart(), HEALTH_COLUMNS, HorizontalBars(), Sparkline(), StatusGrid(), DashboardKpiCard(), CAPA_FILTERS (+11 more)

### Community 22 - "wacrm_client.py"
Cohesion: 0.06
Nodes (8): react, FormInput(), Notification(), TYPE_STYLES, SlaMeter(), STAGE_ACTIONS, Button(), buttonVariants

### Community 23 - "Top Navigation Bar"
Cohesion: 0.11
Nodes (14): new_user_id(), ensure_headers(), Raised when the live spreadsheet cannot be read after retrying., Read canonical records while tolerating old, extra, or blank columns.      Produ, Append newly introduced schema columns without disturbing existing data., read_records(), SheetsUnavailableError, _normalize() (+6 more)
...
Cohesion: 0.05
Nodes (52): approve_ticket_closure(), delegate_to_colleague(), escalate_part_request(), escalate_ticket(), initialize_ticket_escalation(), mark_outsourced(), _minutes_open(), Escalation engine — checks ticket timers and fires WhatsApp notifications.  Two (+44 more)
...
Cohesion: 0.10
Nodes (44): _content_matches_extension(), FileTooLargeError, ABC, Pluggable file storage — local disk (dev/test) and Google Drive (production).  T, Check that file content magic bytes match the claimed extension., Shared upload validation — called before any save attempt., UnsupportedFileTypeError, validate_upload() (+36 more)
...
Cohesion: 0.10
Nodes (13): Sends a downloaded voice note to OpenAI's transcription API and returns the, transcribe_audio(), _cell_value(), main(), Creates the live Google Sheet ("TurboFix-Tracker-Live") from TurboFix-Tracker.xl, admin_token(), test_onboard_company_duplicate_rejected(), test_onboard_company_invalid_password_rejected() (+5 more)
...
Cohesion: 0.05
Nodes (42): Self-contained internal TurboFix platform administration console., create_admin_token(), Rate limiting for sensitive endpoints — prevents brute-force, spam, and DoS., admin_company_dashboard(), admin_company_users(), admin_company_workspace_preview(), admin_console(), admin_list_companies() (+34 more)
...
Cohesion: 0.05
Nodes (49): TechnicianWorkRepository, CustomKpiRepository, AntDChartCard(), AntDDetailList(), AntDEmptyState(), AntDKPICard(), ClosedLoopControlCard(), AdvancedAnalyticsBoard() (+41 more)
...
Cohesion: 0.14
Nodes (23): create_access_token(), get_custom_kpis(), get_events(), get_technician_work(), get_tickets(), get_users(), Return the configured EventRepository implementation (cached singleton)., Return the configured UserRepository implementation (cached singleton). (+15 more)
...
## Knowledge Gaps
- **647 isolated node(s):** `ROLE_HEADINGS`, `STATUS_FILTERS`, `EMPTY_FILTERS`, `bgSyncPlugin`, `SHELF_PARTS` (+642 more)
...
- **What is the exact relationship between `Bluesky Icon (butterfly logo)` and `Social/Community Icon (person with badge/ribbon, purple stroke)`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `react` connect `wacrm_client.py` to `SupabaseCustomKpiRepository`, `machine_record_service.py`, `urlEncryption.js`, `fanout_service.py`, `LocalTicketRepository`, `Home.jsx`, `Prerequisites`, `MachineRepository`, `kpi_router.py`, `config.py`, `react`, `Machines.jsx`, `_factory_id_for_code`, `qrgateway-utils.test.js`, `SecurityHeadersMiddleware`, `ai_assistant/index.ts`, `intelligence_service.py`, `utils.js`, `provider.py`, `SupabaseFileStorage`, `useI18n`, `PerformanceMonitor`, `dynamicChecklist.js`, `SheetsCustomKpiRepository`, `machine_data_service.py`, `Inventory.jsx`, `LocalDocumentRepository`, `LocalTechnicianWorkRepository`, `FakeAsyncClient`, `.oxlintrc.json`, `accessibility.js`, `ErrorBoundary`, `tabs.jsx`, `roles.js`, `AntDProvider.jsx`, `DriveFileStorage`?**
  _High betweenness centrality (0.046) - this node is a cross-community bridge._
- **Why does `TicketRepository` connect `UserRepository` to `SupabaseMachineRecordRepository`, `whatsapp_webhook/index.ts`, `LocalTechnicianWorkRepository`, `TurboFix WhatsApp Brochure (Marketing Image)`, `auth_headers`, `main.py`, `TicketRepository`, `LocalUserRepository`, `base.py`, `test_new_features.py`, `.get`, `dependencies.py`, `test_webhook.py`, `dependencies`, `get_tickets`, `ticket_gateway/index.ts`, `_rate_limit_handler`, `test_summarize.py`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
- **Why does `MachineRepository` connect `dependencies.py` to `UserRepository`, `whatsapp_webhook/index.ts`, `LocalTechnicianWorkRepository`, `auth_headers`, `main.py`, `TicketRepository`, `base.py`, `test_new_features.py`, `dependencies`, `TurboFix WhatsApp Brochure (Marketing Image)`, `test_webhook.py`, `escalation_service.py`, `LocalUserRepository`, `.get`, `get_tickets`, `ticket_gateway/index.ts`, `SupabaseMachineRecordRepository`, `test_admin_onboarding.py`, `test_summarize.py`?**
