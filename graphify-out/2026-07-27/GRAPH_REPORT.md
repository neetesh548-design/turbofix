...
## Corpus Check
- 473 files · ~572,600 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3742 nodes · 7955 edges · 289 communities (206 shown, 83 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 658 edges (avg confidence: 0.58)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `910d25ba`
...
## Communities (289 total, 83 thin omitted)
...
Cohesion: 0.06
Nodes (76): HistorySuggestion(), IssueCapture(), QUICK_PHRASES, MachineSelector(), scannerAvailable(), compressImage(), PhotoCapture(), RoleForm() (+68 more)
...
### Community 20 - "react"
Cohesion: 0.09
Nodes (42): CRITICALITY_ROWS, DashboardChart(), HEALTH_COLUMNS, HorizontalBars(), Sparkline(), StatusGrid(), DashboardKpiCard(), CAPA_FILTERS (+34 more)

### Community 22 - "wacrm_client.py"
Cohesion: 0.04
Nodes (12): react, FormInput(), Notification(), TYPE_STYLES, SlaMeter(), STAGE_ACTIONS, Badge(), badgeVariants (+4 more)
...
Cohesion: 0.12
Nodes (14): AppShell(), getLiveDataAnswer(), isTokenExpired(), NAV_LIVE, readAuth(), Assistant(), getLiveDataAnswer(), machineSuggestions (+6 more)
...
Cohesion: 0.21
Nodes (12): fetchKaizenSources(), fetchWithTimeout(), Kaizen(), realizeSaving(), ROLE_HEADINGS, createMetricsCache(), DEMO_KAIZENS, DEMO_OPERATOR (+4 more)
...
Cohesion: 0.10
Nodes (20): Footer(), LanguageGate(), MicrosoftAppLauncher(), Navbar(), SkipLink(), LanguageContext, LanguageProvider(), useLanguage() (+12 more)
...
Cohesion: 0.22
Nodes (15): CustomKpiRepository, ClosedLoopControlCard(), Dashboard(), fetchRoleSources(), fetchWithTimeout(), ROLE_HEADINGS, buildDemoTickets(), daysAgo() (+7 more)
...
### Community 279 - "dashboardLayout.js"
Cohesion: 0.29
...
Cohesion: 0.08
Nodes (14): TechnicianWorkRepository, AdvancedFeaturesDrilldown(), ContactReveal(), EmptyState(), MachineDetailDrawer(), URGENCY_TONE, KAIZEN_CATEGORIES, LEAN_WASTES (+6 more)
...
## Knowledge Gaps
- **651 isolated node(s):** `DEMO_PARTS`, `DEMO_CONSUMABLES`, `DEMO_PURCHASE_ORDERS`, `DEMO_SUPPLIERS`, `SHELF_PARTS` (+646 more)
...
- **What is the exact relationship between `Bluesky Icon (butterfly logo)` and `Social/Community Icon (person with badge/ribbon, purple stroke)`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `react` connect `wacrm_client.py` to `SupabaseCustomKpiRepository`, `machine_record_service.py`, `urlEncryption.js`, `fanout_service.py`, `LocalTicketRepository`, `Home.jsx`, `Prerequisites`, `demoDashboard.js`, `kpi_router.py`, `config.py`, `react`, `Machines.jsx`, `_factory_id_for_code`, `qrgateway-utils.test.js`, `SecurityHeadersMiddleware`, `ai_assistant/index.ts`, `intelligence_service.py`, `provider.py`, `SupabaseFileStorage`, `PerformanceMonitor`, `dynamicChecklist.js`, `SheetsCustomKpiRepository`, `machine_data_service.py`, `Inventory.jsx`, `LocalDocumentRepository`, `LocalTechnicianWorkRepository`, `FakeAsyncClient`, `.oxlintrc.json`, `accessibility.js`, `tabs.jsx`, `roles.js`, `Tickets.jsx`, `DriveFileStorage`?**
  _High betweenness centrality (0.039) - this node is a cross-community bridge._
- **Why does `TicketRepository` connect `main.py` to `SupabaseMachineRecordRepository`, `UserRepository`, `LocalTechnicianWorkRepository`, `TicketRepository`, `ErrorBoundary`, `LocalUserRepository`, `base.py`, `dependencies.py`, `test_webhook.py`, `dependencies`, `get_tickets`, `whatsapp.py`, `_rate_limit_handler`, `test_summarize.py`?**
  _High betweenness centrality (0.025) - this node is a cross-community bridge._
- **Why does `MachineRepository` connect `UserRepository` to `SupabaseMachineRecordRepository`, `LocalTechnicianWorkRepository`, `escalation_service.py`, `main.py`, `TicketRepository`, `ErrorBoundary`, `LocalUserRepository`, `base.py`, `dependencies.py`, `dependencies`, `get_tickets`, `whatsapp.py`, `test_webhook.py`, `test_summarize.py`?**
