...
## Corpus Check
- 473 files · ~571,936 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3742 nodes · 7953 edges · 287 communities (205 shown, 82 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 650 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4caa71e9`
...
## Communities (287 total, 82 thin omitted)

### Community 0 - "UserRepository"
Cohesion: 0.04
Nodes (93): Self-contained internal TurboFix platform administration console., create_admin_token(), Role, MachineRepository, ABC, Read/write access to the Machines data entity., Return {machine_id: {...}} for all machines (may be cached)., Return the machine dict, or None if not found. (+85 more)
...
Cohesion: 0.05
Nodes (63): create_reset_token(), decode_access_token(), decode_reset_token(), get_current_admin(), get_current_user(), hash_password(), _password_fingerprint(), Phase 5 - Document Vault authentication.  A small, real (not stubbed) JWT auth l (+55 more)
...
### Community 9 - "main.py"
Cohesion: 0.04
Nodes (85): EventRepository, new_event_id(), Read/write access to the Tickets data entity., Generate a new unique ticket ID., Append a new ticket row. Keys must match TICKETS_HEADER., Return the ticket dict for ticket_id, or None if not found., Set voice_note_media_id on the matching row. Returns True if found., Update AI-generated fields on the matching ticket. Returns True if found. (+77 more)
...
Cohesion: 0.11
Nodes (16): _content_matches_extension(), DriveFileStorage, FileTooLargeError, LocalFileStorage, _object_key(), ABC, Path, Pluggable file storage — local disk (dev/test) and Google Drive (production).  T (+8 more)
...
Cohesion: 0.05
Nodes (14): react, FormInput(), Notification(), TYPE_STYLES, SlaMeter(), STAGE_ACTIONS, Alert(), alertVariants (+6 more)
...
Cohesion: 0.14
Nodes (9): AdvancedFeaturesDrilldown(), AppShell(), getLiveDataAnswer(), isTokenExpired(), NAV_LIVE, readAuth(), EmptyState(), AccordionSections() (+1 more)
...
Cohesion: 0.06
Nodes (70): get_file_storage(), Return the FileStorage implementation selected by DOCUMENT_STORE env var., DocumentRepository, MachineRecordRepository, Read/write access to the Documents metadata entity., Generate a new unique document ID., Return all documents for a company (optionally filtered by machine)., Return the document dict, or None. (+62 more)
...
Cohesion: 0.10
Nodes (27): is_configured(), _graph_url(), WhatsApp messaging — routes through WaCRM when configured, else direct Meta Clou, Send a plain text message., Send turbofix_escalation in its six-placeholder order., Send a broadcast to multiple recipients. Only available via WaCRM.      recipien, Send a Meta template with body values in placeholder order., Send turbofix_ticket_closed: ticket, machine, resolution, closer, duration. (+19 more)
...
Cohesion: 0.11
Nodes (24): _auto_reorder_loop(), _daily_digest_loop(), _drift_check_loop(), _escalation_loop(), health(), _lifespan(), _predictive_loop(), TurboFix backend entry point — thin application bootstrap.  This module's only j (+16 more)
...
Cohesion: 0.13
Nodes (21): Pluggable outbound email (Phase 5 - password reset).  Mirrors the local/sheets a, Send (or, in console mode, log) a plain-text email. Failures are logged and, _send_console(), send_email(), _send_smtp(), configure_logging(), get_logger(), Structured JSON logging for TurboFix, backed by structlog.  Every log event prod (+13 more)
...
Cohesion: 0.16
Nodes (13): TechnicianWorkRepository, OptimizedList(), Assistant(), getLiveDataAnswer(), machineSuggestions, plantSuggestions, issueText(), roleTypes (+5 more)
...
Cohesion: 0.09
Nodes (53): StockHealthIndicator(), stockReason(), daysAgo(), makePart(), NOW, one(), po(), PO_ROWS (+45 more)
...
Cohesion: 0.04
Nodes (59): get_ai_feedback(), get_custom_kpis(), get_documents(), get_escalation_config(), get_events(), get_machine_records(), get_machines(), get_part_requests() (+51 more)
...
Cohesion: 0.11
Nodes (9): ContactReveal(), MachineDetailDrawer(), URGENCY_TONE, KAIZEN_CATEGORIES, LEAN_WASTES, PRE_SEEDED_KAIZENS, WORKSPACE_TABS, DEMO_MACHINES (+1 more)
...
## Knowledge Gaps
- **652 isolated node(s):** `ROLE_HEADINGS`, `STATUS_FILTERS`, `EMPTY_FILTERS`, `ROLE_VIEW_MAP`, `PRIORITY_WEIGHT` (+647 more)
...
- **What is the exact relationship between `Bluesky Icon (butterfly logo)` and `Social/Community Icon (person with badge/ribbon, purple stroke)`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `react` connect `wacrm_client.py` to `SupabaseCustomKpiRepository`, `machine_record_service.py`, `urlEncryption.js`, `fanout_service.py`, `LocalTicketRepository`, `Home.jsx`, `Prerequisites`, `demoDashboard.js`, `kpi_router.py`, `config.py`, `react`, `Machines.jsx`, `qrgateway-utils.test.js`, `SecurityHeadersMiddleware`, `ai_assistant/index.ts`, `intelligence_service.py`, `utils.js`, `provider.py`, `SupabaseFileStorage`, `PerformanceMonitor`, `dynamicChecklist.js`, `SheetsCustomKpiRepository`, `machine_data_service.py`, `Inventory.jsx`, `LocalDocumentRepository`, `ticket_gateway/index.ts`, `LocalTechnicianWorkRepository`, `FakeAsyncClient`, `.oxlintrc.json`, `accessibility.js`, `tabs.jsx`, `roles.js`, `Tickets.jsx`, `DriveFileStorage`?**
  _High betweenness centrality (0.039) - this node is a cross-community bridge._
- **Why does `UserRepository` connect `UserRepository` to `SupabaseMachineRecordRepository`, `SheetsMachineRecordRepository`, `LocalTechnicianWorkRepository`, `whatsapp_webhook/index.ts`, `escalation_service.py`, `LocalUserRepository`, `Top Navigation Bar`, `get_tickets`?**
  _High betweenness centrality (0.020) - this node is a cross-community bridge._
- **Why does `TicketRepository` connect `main.py` to `SupabaseMachineRecordRepository`, `UserRepository`, `LocalTechnicianWorkRepository`, `whatsapp_webhook/index.ts`, `test_admin_onboarding.py`, `escalation_service.py`, `TicketRepository`, `LocalUserRepository`, `base.py`, `test_webhook.py`, `dependencies`, `get_tickets`, `whatsapp.py`, `_rate_limit_handler`?**
