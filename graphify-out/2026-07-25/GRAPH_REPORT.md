...
## Corpus Check
- 382 files · ~478,881 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3102 nodes · 6206 edges · 260 communities (184 shown, 76 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 590 edges (avg confidence: 0.56)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `0a53d2ec`
...
## Communities (260 total, 76 thin omitted)

### Community 0 - "UserRepository"
Cohesion: 0.04
Nodes (85): Role, get_technician_work(), Return persistent technician work records using the active store backend., MachineRepository, ABC, Read/write access to the Tickets data entity., Generate a new unique ticket ID., Append a new ticket row. Keys must match TICKETS_HEADER. (+77 more)
...
Cohesion: 0.14
Nodes (26): new_event_id(), ai_enabled(), Return True if any AI provider is configured and ready., _detect_and_store_language(), finish_audio_ticket(), finish_image_ticket(), finish_text_ticket(), handle_close_command() (+18 more)

### Community 3 - "DocumentRepository"
Cohesion: 0.06
Nodes (73): get_documents(), Return the configured DocumentRepository implementation (cached singleton)., get_file_storage(), Return the FileStorage implementation selected by DOCUMENT_STORE env var., DocumentRepository, MachineRecordRepository, Read/write access to the Documents metadata entity., Generate a new unique document ID. (+65 more)
...
Cohesion: 0.06
Nodes (33): Self-contained internal TurboFix platform administration console., create_admin_token(), admin_company_users(), admin_company_workspace_preview(), admin_console(), admin_list_companies(), admin_login(), _company_approved() (+25 more)
...
Cohesion: 0.14
Nodes (12): AdvancedFeaturesDrilldown(), ContactReveal(), EmptyState(), KAIZEN_CATEGORIES, KAIZEN_STATUSES, LEAN_WASTES, PRE_SEEDED_KAIZENS, KAIZEN_CATEGORIES (+4 more)
...
Cohesion: 0.06
Nodes (20): react, Footer(), LanguageGate(), Navbar(), SkipLink(), LanguageContext, LanguageProvider(), useLanguage() (+12 more)
...
Cohesion: 0.11
Nodes (26): download_media(), _graph_url(), WhatsApp messaging — routes through WaCRM when configured, else direct Meta Clou, Send a plain text message., Send turbofix_escalation in its six-placeholder order., Send a broadcast to multiple recipients. Only available via WaCRM.      recipien, Resolve a WhatsApp media ID → download → save to MEDIA_STORE_DIR.      Returns t, Send a Meta template with body values in placeholder order. (+18 more)
...
Cohesion: 0.06
Nodes (28): Sends a downloaded voice note to OpenAI's transcription API and returns the, transcribe_audio(), Local (openpyxl / Excel) implementations of TicketRepository and MachineReposito, get_technician_workload(), check_and_flag_on_creation(), flag_repeat_failure(), get_factory_ai_stats(), get_least_loaded_technician() (+20 more)
...
Cohesion: 0.16
Nodes (19): AntDNavigationLayout(), getNavMenuItems(), getUserMenuItems(), DirectionProvider(), getFlagEmoji(), LanguageStats(), LanguageSwitcher(), LocalizedDate() (+11 more)
...
Cohesion: 0.12
Nodes (23): AppShell(), getLiveDataAnswer(), isTokenExpired(), NAV_LIVE, NAV_SOON, readAuth(), Assistant(), getLiveDataAnswer() (+15 more)
...
Cohesion: 0.12
Nodes (5): new_kpi_entry_id(), new_kpi_id(), LocalCustomKpiRepository, In-memory implementation of CustomKpiRepository for local dev/testing., SheetsCustomKpiRepository
...
Cohesion: 0.04
Nodes (54): get_ai_feedback(), get_custom_kpis(), get_escalation_config(), get_events(), get_machine_records(), get_parts(), get_settings(), get_shift_config() (+46 more)
...
### Community 247 - "report_router.py"
Cohesion: 0.19
Nodes (3): TechnicianWorkRepository, Technician(), I18nManager

### Community 248 - "sweep_expired_unnotified"
Cohesion: 0.24
Nodes (3): compressed_json(), FakePostgrestClient, test_supabase_machine_record_repository_is_tenant_scoped_and_expands_json()

### Community 249 - "TestReports"
Cohesion: 0.29
...
### Community 258 - "DriveFileStorage"
Cohesion: 0.38
...
Cohesion: 0.16
Nodes (4): LocalSettingsRepository, Excel-backed company settings repository for local development and tests., LocalTechnicianWorkRepository, Excel-backed technician work records.
...
## Knowledge Gaps
- **511 isolated node(s):** `fallback`, `money`, `$schema`, `oxc`, `react/rules-of-hooks` (+506 more)
...
- **What is the exact relationship between `Bluesky Icon (butterfly logo)` and `Social/Community Icon (person with badge/ribbon, purple stroke)`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `MachineRepository` connect `UserRepository` to `EventRepository`, `whatsapp_webhook/index.ts`, `DocumentRepository`, `fanout_service.py`, `TicketRepository`, `base.py`, `badge.jsx`, `react`, `dependencies`, `whatsapp.py`, `test_webhook.py`, `escalation_service.py`, `LocalUserRepository`, `sw-strategies.js`, `SupabaseFileStorage`, `performance.jsx`, `report_service.py`, `select.jsx`, `icons.svg (Icon Sprite Sheet)`, `SupabaseMachineRecordRepository`, `test_machine_records.py`, `imports`?**
  _High betweenness centrality (0.037) - this node is a cross-community bridge._
- **Why does `TicketRepository` connect `UserRepository` to `EventRepository`, `whatsapp_webhook/index.ts`, `DocumentRepository`, `fanout_service.py`, `TicketRepository`, `base.py`, `badge.jsx`, `react`, `dependencies`, `whatsapp.py`, `test_webhook.py`, `escalation_service.py`, `LocalUserRepository`, `sw-strategies.js`, `dependencies.py`, `SupabaseFileStorage`, `performance.jsx`, `report_service.py`, `select.jsx`, `icons.svg (Icon Sprite Sheet)`, `SupabaseMachineRecordRepository`, `test_machine_records.py`, `imports`?**
  _High betweenness centrality (0.025) - this node is a cross-community bridge._
- **Why does `react` connect `wacrm_client.py` to `AntDDashboardComponents.jsx`, `LocalTicketRepository`, `MachineRepository`, `test_new_features.py`, `kpi_router.py`, `config.py`, `SheetsTicketRepository`, `inbound_email_receiver/index.ts`, `intelligence_service.py`, `components.json`, `useI18n`, `SheetsPartsRepository`, `dynamicChecklist.js`, `SheetsCustomKpiRepository`, `machine_data_service.py`, `test_supabase_machine_records.py`, `SupabasePartsRepository`, `ticket_gateway/index.ts`, `.oxlintrc.json`, `accessibility.js`, `ErrorBoundary`, `roles.js`, `approve_ticket_closure`, `badge.jsx`, `onboard_team_member/index.ts`?**
