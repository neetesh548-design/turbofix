...
## Corpus Check
- 385 files · ~485,099 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3254 nodes · 6301 edges · 271 communities (194 shown, 77 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 590 edges (avg confidence: 0.56)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `136fad46`
...
Cohesion: 0.22
Nodes (15): admin_company_workspace_preview(), _company_quota(), Return a safe, read-only representation of the customer's workspace., list_machines(), list_team(), reveal_team_contact(), can_reveal_contact(), company_hierarchy() (+7 more)
...
Cohesion: 0.07
Nodes (28): CustomKpiRepository, AntDChartCard(), AntDDashboardComponentsExports, AntDDetailList(), AntDEmptyState(), AntDKPICard(), ClosedLoopControlCard(), asNumber() (+20 more)

### Community 14 - "test_new_features.py"
Cohesion: 0.03
Nodes (75): get_ai_feedback(), get_custom_kpis(), get_escalation_config(), get_part_requests(), get_shift_config(), Dependency Injection factories for all TurboFix repositories.  FastAPI's Depends, Return the configured CustomKpiRepository implementation (cached singleton)., CustomKpiRepository (+67 more)

### Community 15 - "Dashboard.jsx"
Cohesion: 0.05
Nodes (67): get_documents(), get_events(), get_machines(), get_parts(), get_settings(), get_technician_work(), get_tickets(), get_users() (+59 more)
...
### Community 20 - "react"
Cohesion: 0.08
Nodes (37): MachineRepository, new_document_id(), Read/write access to the Machines data entity., Return {machine_id: {...}} for all machines (may be cached)., Return the machine dict, or None if not found., Append a new machine row. Keys must match MACHINES_HEADER (minus has_open_ticket, Force the next load() to re-read from the backing store., Return the next Mnnn code for a company, e.g. 'M003'. (+29 more)
...
Cohesion: 0.09
Nodes (29): is_configured(), _graph_url(), WhatsApp messaging — routes through WaCRM when configured, else direct Meta Clou, Send turbofix_escalation in its six-placeholder order., Send a broadcast to multiple recipients. Only available via WaCRM.      recipien, Send a Meta template with body values in placeholder order., Send turbofix_new_ticket: ticket, machine, location, issue, urgency, assignee., Send turbofix_ticket_closed: ticket, machine, resolution, closer, duration. (+21 more)
...
Cohesion: 0.13
Nodes (19): Pluggable outbound email (Phase 5 - password reset).  Mirrors the local/sheets a, Send (or, in console mode, log) a plain-text email. Failures are logged and, _send_console(), send_email(), _send_smtp(), _is_retryable(), Resilient HTTP client with tenacity retry + exponential backoff.  Wraps httpx fo, get_logger() (+11 more)
...
Cohesion: 0.10
Nodes (25): get_downtime_summary(), get_ticket_downtime(), acknowledge_prediction(), calculate_ticket_downtime_cost(), _check_failure_pattern(), _create_prediction(), get_factory_downtime_summary(), get_predictions() (+17 more)
...
Cohesion: 0.11
Nodes (8): download_media(), Resolve a WhatsApp media ID → download → save to MEDIA_STORE_DIR.      Returns t, handle_closure_evidence(), Technician sends evidence photo for closure — forward to Maintenance Head., FakeAsyncClient, FakePostAsyncClient, FakeResponse, test_download_media_saves_file()

### Community 48 - ".get"
Cohesion: 0.10
Nodes (19): initialize_ticket_escalation(), Set the first escalation timer on a newly created ticket., Integration tests for escalation_service.py  Tests ticket escalation workflow: -, Test WhatsApp message formatting for escalation alerts., Test escalation when spare parts unavailable in stock., In-memory session store for testing., Test that _escalation_loop runs without errors., Test escalation timing respects factory shift schedule. (+11 more)
...
Cohesion: 0.08
Nodes (23): TechnicianWorkRepository, AppShell(), getLiveDataAnswer(), isTokenExpired(), NAV_LIVE, _NAV_SOON, readAuth(), Assistant() (+15 more)
...
Cohesion: 0.05
Nodes (62): _content_matches_extension(), DriveFileStorage, FileStorage, FileTooLargeError, LocalFileStorage, _object_key(), ABC, Path (+54 more)
...
Cohesion: 0.04
Nodes (89): Self-contained internal TurboFix platform administration console., create_admin_token(), create_reset_token(), decode_access_token(), decode_reset_token(), get_current_admin(), get_current_user(), hash_password() (+81 more)
...
Cohesion: 0.24
Nodes (11): admin_company_dashboard(), build_custom_kpi_values(), compute_auto_insights(), compute_kpis(), _parse_dt(), datetime, Dashboard service — compute per-company KPIs from live ticket/machine data.  Ext, Derive MTBF, MTTR, repeat breakdown %, and top problem machines from ticket data (+3 more)
...
Cohesion: 0.07
Nodes (24): Sends a downloaded voice note to OpenAI's transcription API and returns the, transcribe_audio(), Local (openpyxl / Excel) implementations of TicketRepository and MachineReposito, get_technician_workload(), get_factory_ai_stats(), get_least_loaded_technician(), get_machine_ai_accuracy(), get_shift_config() (+16 more)
...
Cohesion: 0.33
Nodes (6): delegate_to_colleague(), Delegate ticket to a colleague; resets the escalation timer., handle_delegate_command(), Delegate a ticket to a colleague., Test technician delegating ticket to colleague., test_delegate_to_colleague()

### Community 154 - "inbound_email_receiver/index.ts"
Cohesion: 0.17
Nodes (15): escalate_part_request(), escalate_ticket(), _minutes_open(), Escalation engine — checks ticket timers and fires WhatsApp notifications.  Two, Advance a part request to the next escalation level., Technician submits evidence photo; notify Maintenance Head for approval., Check all open tickets and part requests for overdue escalations., Advance a single ticket to the next escalation level. (+7 more)
...
Cohesion: 0.33
Nodes (6): mark_outsourced(), Manager marks ticket as outsourced; escalation pauses., handle_outsource_command(), Manager marks ticket as outsourced to a third-party vendor., Test marking issue as outsourced (vendor/contractor involvement)., test_mark_outsourced_escalation()
...
### Community 241 - "radix-ui"
Cohesion: 0.29
Nodes (6): MachineRepository, KAIZEN_CATEGORIES, LEAN_WASTES, Machines(), PRE_SEEDED_KAIZENS, WORKSPACE_TABS

### Community 243 - "Kaizen.jsx"
Cohesion: 0.33
Nodes (4): _KAIZEN_CATEGORIES, _KAIZEN_STATUSES, LEAN_WASTES, PRE_SEEDED_KAIZENS
...
### Community 252 - "_rate_limit_handler"
Cohesion: 0.50
...
Cohesion: 0.33
Nodes (6): Maintenance Head rejects closure — ticket reopened, technician notified., reject_ticket_closure(), handle_reject_command(), Maintenance Head rejects closure evidence., Test supervisor rejecting technician's closure (incomplete work)., test_reject_ticket_closure_with_reason()
...
## Knowledge Gaps
- **623 isolated node(s):** `Executive Summary`, `Quality Metrics`, `1. **UI Components**`, `2. **Core Logic**`, `3. **Test Suite**` (+618 more)
...
- **What is the exact relationship between `Bluesky Icon (butterfly logo)` and `Social/Community Icon (person with badge/ribbon, purple stroke)`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `react` connect `config.py` to `LocalTicketRepository`, `button.jsx`, `MachineRepository`, `kpi_router.py`, `wacrm_client.py`, `intelligence_service.py`, `utils.js`, `components.json`, `useI18n`, `dynamicChecklist.js`, `SheetsCustomKpiRepository`, `machine_data_service.py`, `OfflineQueue`, `ticket_gateway/index.ts`, `SupabaseMachineRecordRepository`, `.oxlintrc.json`, `accessibility.js`, `ErrorBoundary`, `tabs.jsx`, `roles.js`, `radix-ui`, `Kaizen.jsx`, `QRGateway.jsx`, `onboard_team_member/index.ts`, `AntDNavigationLayout.jsx`?**
  _High betweenness centrality (0.027) - this node is a cross-community bridge._
- **Why does `MachineRecordRepository` connect `UserRepository` to `test_machine_records.py`, `report_service.py`, `fanout_service.py`, `LocalUserRepository`, `test_new_features.py`, `SheetsPartsRepository`, `whatsapp.py`, `LocalTechnicianWorkRepository`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Why does `UserRepository` connect `report_service.py` to `UserRepository`, `SheetsMachineRecordRepository`, `fanout_service.py`, `LocalUserRepository`, `test_new_features.py`, `Dashboard.jsx`, `react`, `Top Navigation Bar`, `imports`?**
