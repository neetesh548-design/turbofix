...
## Corpus Check
- 518 files · ~606,286 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4006 nodes · 8297 edges · 305 communities (221 shown, 84 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 543 edges (avg confidence: 0.6)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9ae2f8dc`
...
1. `react` - 130 edges
2. `MachineRepository` - 111 edges
3. `CurrentUser` - 103 edges
4. `TicketRepository` - 96 edges
5. `auth_headers()` - 78 edges
6. `login()` - 74 edges
7. `UserRepository` - 73 edges
...
Cohesion: 0.08
Nodes (40): get_ai_feedback(), get_custom_kpis(), get_documents(), get_escalation_config(), get_events(), get_machine_records(), get_machines(), get_parts() (+32 more)
...
Cohesion: 0.03
Nodes (100): CustomKpiRepository, MachineRepository, ABC, Read/write access to the Tickets data entity., Generate a new unique ticket ID., Append a new ticket row. Keys must match TICKETS_HEADER., Return the ticket dict for ticket_id, or None if not found., Set voice_note_media_id on the matching row. Returns True if found. (+92 more)
...
### Community 9 - "main.py"
Cohesion: 0.06
Nodes (63): EventRepository, Read/write access to the MachineEvents data entity., Append a new event row. Keys must match MACHINE_EVENTS_HEADER., Return all events for a machine, oldest first., Return all events for a company., BackgroundTasks, Request, Receive webhook events from WaCRM.      WaCRM fires events for: message.received (+55 more)
...
Cohesion: 0.13
Nodes (15): DigestConfigRequest, EscalationThresholdRequest, GeminiConfigUpdate, ShiftConfigRequest, update_gemini_config(), upsert_digest_config(), upsert_escalation_threshold(), upsert_shift() (+7 more)
...
Cohesion: 0.10
Nodes (14): Read/write access to company-scoped operational settings., Return the settings row for a company, or None when not configured., Create or replace the settings row for one company., SettingsRepository, LocalSettingsRepository, Excel-backed company settings repository for local development and tests., SupabaseCustomKpiRepository, add_custom_role() (+6 more)
...
Cohesion: 0.07
Nodes (11): new_user_id(), LocalUserRepository, _normalize(), Local (openpyxl / Excel) implementation of UserRepository., Reads/writes Users and Companies tabs in the local tracker workbook., Look up a user by phone or email (case-insensitive, whitespace-trimmed)., _normalize(), Reads/writes Users and Companies worksheets in a Google Sheet. (+3 more)
...
Cohesion: 0.05
Nodes (42): Any, new_event_id(), new_item_id(), new_kpi_entry_id(), new_kpi_id(), new_ticket_id(), Abstract base classes (interfaces) for all TurboFix data repositories.  Every co, LocalCustomKpiRepository (+34 more)

### Community 30 - "test_webhook.py"
Cohesion: 0.08
Nodes (36): get_sessions(), Dependency that returns the module-level session store., Tracks, per sender phone number, the most recent ticket opened from a text     m, Records that this phone's session has already been fanned out, so a later, Removes every expired session (regardless of notified status, so memory, Session, SessionStore, In-memory session store for testing. (+28 more)
...
Cohesion: 0.04
Nodes (116): CurrentUser, The authenticated caller's identity, parsed straight from the JWT - no extra, Enforces the same multi-tenant isolation used for tickets/machines         elsew, _content_matches_extension(), FileStorage, FileTooLargeError, get_file_storage(), Return file bytes for a storage_path previously returned by save(). (+108 more)
...
Cohesion: 0.15
Nodes (16): HistorySuggestion(), compressImage(), PhotoCapture(), EMPTY_DRAFT, fetchBreakdownSources(), fetchWithTimeout(), normaliseMachine(), normaliseReport() (+8 more)

### Community 45 - "SessionStore"
Cohesion: 0.67
Nodes (3): admin_login(), AdminLoginRequest, Request

### Community 46 - "conftest.py"
Cohesion: 0.07
Nodes (26): is_configured(), download_media(), _graph_url(), WhatsApp messaging — routes through WaCRM when configured, else direct Meta Clou, Send a plain text message., Send turbofix_escalation in its six-placeholder order., Send a broadcast to multiple recipients. Only available via WaCRM.      recipien, Resolve a WhatsApp media ID → download → save to MEDIA_STORE_DIR.      Returns t (+18 more)
...
Cohesion: 0.09
Nodes (34): active_provider(), analyze_image(), detect_language(), enabled(), extract_machine_record(), maintenance_assistant(), Resolves which AI backend to use: "gemini", "openai", or "" (AI layer off)., Extract structured maintenance facts without approving them for AI use. (+26 more)
...
Cohesion: 0.03
Nodes (89): ABC, Pluggable file storage — local disk (dev/test) and Google Drive (production).  T, configure_logging(), get_logger(), Structured JSON logging for TurboFix, backed by structlog.  Every log event prod, Call once at application startup (from main.py lifespan)., Return a structlog logger bound to `name`.      The returned logger behaves exac, Rate limiting for sensitive endpoints — prevents brute-force, spam, and DoS. (+81 more)

### Community 85 - "LocalDocumentRepository"
Cohesion: 0.11
Nodes (14): TechnicianWorkRepository, AdvancedFeaturesDrilldown(), AppShell(), getLiveDataAnswer(), ContactReveal(), EmptyState(), MachineDetailDrawer(), URGENCY_TONE (+6 more)

### Community 88 - "test_contact_privacy.py"
Cohesion: 0.09
Nodes (33): analyze_image(), detect_language(), extract_machine_record(), _headers(), maintenance_assistant(), Calls Gemini to turn a raw issue description into a structured brief.     Same p, Send a machine photo to Gemini and get a text description of visible issues., Detect the language and return an ISO 639-1 code. (+25 more)
...
Cohesion: 0.18
Nodes (13): IssueCapture(), QUICK_PHRASES, PhotoAnnotatorModal(), QuickReportDialog(), classifyIssue(), ISSUE_CATEGORIES, KEYWORD_PATTERNS, keywordPattern() (+5 more)
...
Cohesion: 0.05
Nodes (46): approve_ticket_closure(), delegate_to_colleague(), escalate_part_request(), escalate_ticket(), initialize_ticket_escalation(), mark_outsourced(), _minutes_open(), Escalation engine — checks ticket timers and fires WhatsApp notifications.  Two (+38 more)
...
Cohesion: 0.07
Nodes (50): create_access_token(), create_reset_token(), decode_access_token(), decode_reset_token(), get_current_admin(), get_current_user(), hash_password(), _password_fingerprint() (+42 more)
...
Cohesion: 0.12
Nodes (42): MachineSelector(), scannerAvailable(), ReportBreakdown(), machine(), NOW, person(), assignedEngineer(), assignedSupervisor() (+34 more)
...
Cohesion: 0.33
Nodes (6): check_and_flag_on_creation(), check_repeat_failure(), flag_repeat_failure(), Check if machine has had more than `threshold` tickets in last `days` days., Mark a ticket as a repeat failure case., Called after ticket creation to detect repeat failures.

### Community 277 - "test_sheets_user_repo.py"
Cohesion: 0.08
Nodes (5): Admin router — internal TurboFix platform operations console., wacrm_create_contact(), wacrm_send_message(), WaCRMContactCreate, WaCRMSendMessage
...
Cohesion: 0.32
Nodes (11): get_tickets(), Return the configured TicketRepository implementation (cached singleton)., _open_ticket(), _technician_token(), test_evidence_upload_is_persisted_and_downloadable(), test_submission_requires_complete_checklist_and_notes(), test_technician_can_submit_work_for_supervisor_approval(), test_unassigned_technician_cannot_access_ticket() (+3 more)
...
- **Are the 18 inferred relationships involving `CurrentUser` (e.g. with `AddSupervisorRequest` and `ForgotPasswordRequest`) actually correct?**
  _`CurrentUser` has 18 INFERRED edges - model-reasoned connections that need verification._