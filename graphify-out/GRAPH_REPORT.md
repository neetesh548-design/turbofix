...
## Corpus Check
- 544 files · ~649,825 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4264 nodes · 8058 edges · 367 communities (231 shown, 136 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 363 edges (avg confidence: 0.64)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f4685a5b`
...
## Communities (367 total, 136 thin omitted)
...
Cohesion: 0.05
Nodes (53): ABC, MachineRepository, Read/write access to the Machines data entity., Return {machine_id: {...}} for all machines (may be cached)., Return the machine dict, or None if not found., Append a new machine row. Keys must match MACHINES_HEADER (minus has_open_ticket, Force the next load() to re-read from the backing store., Return the next Mnnn code for a company, e.g. 'M003'. (+45 more)
...
### Community 9 - "main.py"
Cohesion: 0.06
Nodes (45): Pluggable outbound email (Phase 5 - password reset).  Mirrors the local/sheets a, Send (or, in console mode, log) a plain-text email. Failures are logged and, _send_console(), send_email(), _send_smtp(), configure_logging(), get_logger(), Structured JSON logging for TurboFix, backed by structlog.  Every log event prod (+37 more)
...
Cohesion: 0.08
Nodes (23): Integration tests for dashboard_service.py  Tests KPI computation and dashboard, Test total count of open tickets., Test calculation of average hours to fix (resolved tickets)., Sample ticket data for dashboard tests., Test that dashboard is isolated per company., Test calculation of a single custom KPI., Test KPI with multiple field references., Test KPI calculation handles division by zero gracefully. (+15 more)
...
Cohesion: 0.02
Nodes (12): react, compressImage(), PhotoCapture(), TREND_COPY, ErrorBoundary, FormInput(), URGENCY_TONE, Notification() (+4 more)

### Community 23 - "Top Navigation Bar"
Cohesion: 0.11
Nodes (19): machine(), NOW, person(), categoryMeta(), confirmationMessage(), formatResponseWindow(), GENERAL_CATEGORY, nextWorkOrderNumber() (+11 more)
...
Cohesion: 0.14
Nodes (12): ensure_headers(), _get_all_values(), Read canonical records while tolerating old, extra, or blank columns.      Produ, Append newly introduced schema columns without disturbing existing data., read_records(), _normalize(), Google Sheets implementation of UserRepository., Reads/writes Users and Companies worksheets in a Google Sheet. (+4 more)
...
Cohesion: 0.18
Nodes (16): Any, dispatch_daily_digest(), dispatch_weekly_digest(), format_daily_digest_message(), format_weekly_digest_message(), generate_daily_digest_payload(), generate_weekly_digest_payload(), get_company_executive_phones() (+8 more)
...
Cohesion: 0.20
Nodes (15): get_machine_events(), get_root_cause_analysis(), Return the full event history for a machine (scoped to user's company)., Run AI root cause analysis on a machine's full event history., _approved_record_markdown(), build_machine_data(), _document_text(), _internet_reference() (+7 more)
...
Cohesion: 0.05
Nodes (63): get_current_user(), get_custom_kpis(), get_documents(), get_events(), get_machine_records(), get_machines(), get_parts(), get_settings() (+55 more)
...
Cohesion: 0.22
Nodes (16): MachineSelector(), scannerAvailable(), assignedEngineer(), assignedSupervisor(), assignedTechnician(), buildBreakdownRecord(), isAtLeastUrgent(), machineFromQr() (+8 more)

### Community 86 - "LocalCustomKpiRepository"
Cohesion: 0.11
Nodes (17): SupabaseSettingsRepository, SupabaseTechnicianWorkRepository, admin_company_workspace_preview(), admin_list_companies(), admin_update_company(), _company_approved(), _company_quota(), CompanyUpdate (+9 more)
...
Cohesion: 0.16
Nodes (13): AuditLog(), MOCK_LOGS, IssueCapture(), QUICK_PHRASES, PhotoAnnotatorModal(), QuickReportDialog(), configuredSupabaseUrl, supabase (+5 more)
...
Cohesion: 0.17
Nodes (8): AppShell(), getLiveDataAnswer(), Assistant(), getLiveDataAnswer(), machineSuggestions, plantSuggestions, AccordionSections(), MachinesRefactored()
...
Cohesion: 0.08
Nodes (27): _send_escalation_whatsapp(), check_and_flag_on_creation(), confirm_ai_diagnosis(), extract_machine_record(), flag_repeat_failure(), get_factory_ai_stats(), get_least_loaded_technician(), get_machine_ai_accuracy() (+19 more)
...
Cohesion: 0.14
Nodes (18): _auto_reorder_loop(), _daily_digest_loop(), _drift_check_loop(), _escalation_loop(), health(), _lifespan(), _predictive_loop(), TurboFix backend entry point — thin application bootstrap.  This module's only j (+10 more)
...
Cohesion: 0.27
Nodes (13): CustomKpiRepository, Dashboard(), fetchRoleSources(), fetchWithTimeout(), ROLE_HEADINGS, buildDemoTickets(), daysAgo(), DEMO_PM_LOGS (+5 more)
...
Cohesion: 0.05
Nodes (34): Self-contained internal TurboFix platform administration console., admin_app(), admin_console(), admin_login(), admin_onboard_company(), admin_reset_user_password(), AdminLoginRequest, AdminPasswordResetRequest (+26 more)
...
Cohesion: 0.06
Nodes (37): initialize_ticket_escalation(), Set the first escalation timer on a newly created ticket., Tracks, per sender phone number, the most recent ticket opened from a text     m, Records that this phone's session has already been fanned out, so a later, Removes every expired session (regardless of notified status, so memory, Session, SessionStore, Integration tests for escalation_service.py  Tests ticket escalation workflow: - (+29 more)
...
Cohesion: 0.33
Nodes (6): Background loop: check shift boundaries and send handover digests (every 60s)., _shift_handover_loop(), Send a WhatsApp digest of open tickets to incoming shift technicians., Called periodically — checks if any factory is at a shift boundary., run_shift_handover_check(), send_shift_handover()

### Community 255 - "ErrorBoundary"
Cohesion: 0.18
Nodes (14): _build_role_context(), _fallback_role_response(), generate_ai_chat_response(), handle_registered_whatsapp_message(), normalize_phone(), Role-based WhatsApp Conversational Assistant & AI Query Service for TurboFix.  T, Use Gemini 2.5 Flash to generate a role-appropriate WhatsApp response., Structured fallback response when LLM service is offline. (+6 more)
...
Cohesion: 0.05
Nodes (50): Footer(), LanguageGate(), CapabilityStrip(), ContactCard(), DEFAULT_FAQS, FaqAccordion(), PageHero(), PricingCalculator() (+42 more)
...
Cohesion: 0.09
Nodes (36): Fast2SMS SMS Client — https://docs.fast2sms.com/reference/authorization, send_sms_message(), _rate_limit_handler(), admin_company_users(), add_supervisor(), AddSupervisorRequest, _company_approved(), _extract_10_digit_phone() (+28 more)
...
Cohesion: 0.05
Nodes (68): EventRepository, new_event_id(), Read/write access to the Tickets data entity., Generate a new unique ticket ID., Append a new ticket row. Keys must match TICKETS_HEADER., Return the ticket dict for ticket_id, or None if not found., Set voice_note_media_id on the matching row. Returns True if found., Update AI-generated fields on the matching ticket. Returns True if found. (+60 more)
...
Cohesion: 0.04
Nodes (92): CurrentUser, The authenticated caller's identity, parsed straight from the JWT - no extra, Enforces the same multi-tenant isolation used for tickets/machines         elsew, FileStorage, get_file_storage(), Return file bytes for a storage_path previously returned by save()., Return the FileStorage implementation selected by DOCUMENT_STORE env var., Save file bytes and return an opaque storage_path string for later retrieval. (+84 more)
...
## Knowledge Gaps
- **785 isolated node(s):** `ROLE_HEADINGS`, `WORKSPACE_TABS`, `KAIZEN_CATEGORIES`, `LEAN_WASTES`, `PRE_SEEDED_KAIZENS` (+780 more)
...
- **What is the exact relationship between `Bluesky Icon (butterfly logo)` and `Social/Community Icon (person with badge/ribbon, purple stroke)`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `react` connect `wacrm_client.py` to `machine_record_service.py`, `Support.jsx`, `FakeAuthClient`, `LocalTicketRepository`, `MachineRepository`, `machineIdOf`, `kpi_router.py`, `config.py`, `check_repeat_failure`, `SheetsTicketRepository`, `test_sheets_user_repo.py`, `TurboFix WhatsApp Brochure (Marketing Image)`, `SupabaseEscalationConfigRepository`, `machineExport.js`, `test_vault_auth.py`, `MainLayout.jsx`, `CounterWidget.test.jsx`, `ticketQueues.js`, `intelligence_service.py`, `SupabaseCustomKpiRepository`, `initialize_ticket_escalation`, `scripts`, `health`, `_sheets_unavailable_handler`, `PerformanceMonitor`, `dynamicChecklist.js`, `webhook_router.py`, `button.jsx`, `SheetsCustomKpiRepository`, `machine_data_service.py`, `SupabasePartsRepository`, `LocalMachineRecordRepository`, `LocalTechnicianWorkRepository`, `SupabaseMachineRecordRepository`, `test_admin_onboarding.py`, `FakeAsyncClient`, `.oxlintrc.json`, `Kaizen.jsx`, `tabs.jsx`, `roles.js`, `Dashboard.jsx`, `MachinesRefactored.jsx`?**
  _High betweenness centrality (0.040) - this node is a cross-community bridge._
- **Why does `MachineRepository` connect `fanout_service.py` to `whatsapp_webhook/index.ts`, `DocumentRepository`, `TicketRepository`, `.insert`, `resilient_get`, `conftest.py`, `BaseModel`, `test_sheets_user_repo.py`, `SupabaseTicketRepository`, `dependencies`, `test_webhook.py`?**
  _High betweenness centrality (0.019) - this node is a cross-community bridge._
- **Why does `TicketRepository` connect `.insert` to `DocumentRepository`, `fanout_service.py`, `TicketRepository`, `resilient_get`, `BaseModel`, `test_sheets_user_repo.py`, `SupabaseTicketRepository`, `dependencies`, `test_webhook.py`?**
  _High betweenness centrality (0.019) - this node is a cross-community bridge._
