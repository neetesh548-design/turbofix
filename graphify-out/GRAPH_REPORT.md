...
## Corpus Check
- 341 files · ~419,580 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3353 nodes · 6420 edges · 248 communities (175 shown, 73 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 546 edges (avg confidence: 0.54)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b44566cb`
...
Cohesion: 0.06
Nodes (44): get_technician_work(), Return persistent technician work records using the active store backend., new_document_id(), Read/write access to the Tickets data entity., Generate a new unique ticket ID., Append a new ticket row. Keys must match TICKETS_HEADER., Return the ticket dict for ticket_id, or None if not found., Set voice_note_media_id on the matching row. Returns True if found. (+36 more)
...
Cohesion: 0.05
Nodes (54): get_machine_records(), Return the approved/draft AI machine-record repository., get_file_storage(), Return the FileStorage implementation selected by DOCUMENT_STORE env var., DocumentRepository, MachineRecordRepository, MachineRepository, ABC (+46 more)
...
Cohesion: 0.04
Nodes (89): Self-contained internal TurboFix platform administration console., create_admin_token(), create_reset_token(), decode_reset_token(), get_current_admin(), hash_password(), _password_fingerprint(), Phase 5 - Document Vault authentication.  A small, real (not stubbed) JWT auth l (+81 more)
...
Cohesion: 0.04
Nodes (76): decode_access_token(), get_current_user(), get_ai_feedback(), get_custom_kpis(), get_documents(), get_escalation_config(), get_events(), get_machines() (+68 more)

### Community 13 - "MachineRepository"
Cohesion: 0.06
Nodes (41): configure_logging(), Call once at application startup (from main.py lifespan)., _auto_reorder_loop(), _daily_digest_loop(), _drift_check_loop(), _escalation_loop(), health(), _lifespan() (+33 more)

### Community 15 - "Dashboard.jsx"
Cohesion: 0.08
Nodes (50): EventRepository, new_event_id(), Read/write access to the MachineEvents data entity., Local (openpyxl / Excel) implementations of TicketRepository and MachineReposito, get_machine_events(), get_root_cause_analysis(), Return the full event history for a machine (scoped to user's company)., Run AI root cause analysis on a machine's full event history. (+42 more)
...
Cohesion: 0.10
Nodes (29): is_configured(), download_media(), _graph_url(), WhatsApp messaging — routes through WaCRM when configured, else direct Meta Clou, Send a plain text message., Send turbofix_escalation in its six-placeholder order., Send a broadcast to multiple recipients. Only available via WaCRM.      recipien, Resolve a WhatsApp media ID → download → save to MEDIA_STORE_DIR.      Returns t (+21 more)
...
Cohesion: 0.25
Nodes (9): ask_maintenance_assistant(), _event_line(), _live_data_answer(), _machine_context(), _plant_context(), Answer a maintenance question for one machine or the entire plant., _ticket_line(), maintenance_assistant() (+1 more)

### Community 57 - "useI18n"
Cohesion: 0.12
Nodes (22): Rate limiting for sensitive endpoints — prevents brute-force, spam, and DoS., get_sessions(), _iter_messages(), BackgroundTasks, Request, Webhook router — WhatsApp webhook receive + verify.  This router is intentionall, Verify X-Wacrm-Signature: t=unix,v1=hmac-sha256., Receive webhook events from WaCRM.      WaCRM fires events for: message.received (+14 more)
...
Cohesion: 0.05
Nodes (44): Sends a downloaded voice note to OpenAI's transcription API and returns the, transcribe_audio(), get_logger(), Structured JSON logging for TurboFix, backed by structlog.  Every log event prod, Return a structlog logger bound to `name`.      The returned logger behaves exac, get_downtime_summary(), get_technician_workload(), get_ticket_downtime() (+36 more)
...
Cohesion: 0.08
Nodes (7): react, ContactReveal(), Badge(), badgeVariants, Button(), buttonVariants, ThemeContext
...
## Knowledge Gaps
- **728 isolated node(s):** `1. Comprehensive Audit ✅`, `2. View Mode Toggle Implementation ✅`, `3. AI Records Workflow Discovery ✅`, `Current Status Summary`, `1. WhatsApp Integration (1-2 days) ⬅️ NEXT PRIORITY` (+723 more)
...
- **What is the exact relationship between `Bluesky Icon (butterfly logo)` and `Social/Community Icon (person with badge/ribbon, purple stroke)`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `UserRepository` connect `fanout_service.py` to `UserRepository`, `provider.py`, `CurrentUser`, `LocalUserRepository`, `consumables_service.py`, `SessionStore`, `AppShell.jsx`, `react`, `checklist.md`?**
  _High betweenness centrality (0.027) - this node is a cross-community bridge._
- **Why does `TicketRepository` connect `UserRepository` to `provider.py`, `AntDDashboardComponents.jsx`, `performance.jsx`, `CurrentUser`, `fanout_service.py`, `main.py`, `LocalUserRepository`, `consumables_service.py`, `SessionStore`, `Dashboard.jsx`, `scripts`, `AppShell.jsx`, `Top Navigation Bar`, `SupabaseTicketRepository`, `test_vault_quota_admin.py`, `useI18n`, `api.js`, `.select`?**
  _High betweenness centrality (0.024) - this node is a cross-community bridge._
- **Why does `MachineRepository` connect `CurrentUser` to `UserRepository`, `provider.py`, `performance.jsx`, `EventRepository`, `fanout_service.py`, `main.py`, `LocalUserRepository`, `consumables_service.py`, `SessionStore`, `Dashboard.jsx`, `scripts`, `AppShell.jsx`, `Top Navigation Bar`, `test_vault_quota_admin.py`, `useI18n`, `.select`?**
