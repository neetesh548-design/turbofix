...
## Corpus Check
- 389 files · ~490,498 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3371 nodes · 6404 edges · 276 communities (201 shown, 75 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 590 edges (avg confidence: 0.56)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b4fd868f`
...
Cohesion: 0.04
Nodes (91): download_media(), Resolve a WhatsApp media ID → download → save to MEDIA_STORE_DIR.      Returns t, EventRepository, MachineRepository, Read/write access to the Tickets data entity., Generate a new unique ticket ID., Append a new ticket row. Keys must match TICKETS_HEADER., Return the ticket dict for ticket_id, or None if not found. (+83 more)
...
### Community 9 - "main.py"
Cohesion: 0.06
Nodes (21): extract_machine_record(), maintenance_assistant(), Extract structured machine knowledge from text-readable record sources., Calls OpenAI to turn a raw issue description into a structured brief.     Raises, Answer a scoped maintenance question through the OpenAI provider., summarize_issue(), Sends a downloaded voice note to OpenAI's transcription API and returns the, transcribe_audio() (+13 more)
...
Cohesion: 0.08
Nodes (27): get_ai_feedback(), get_custom_kpis(), get_documents(), get_events(), get_machine_records(), get_parts(), get_settings(), get_shift_config() (+19 more)

### Community 15 - "Dashboard.jsx"
Cohesion: 0.05
Nodes (53): _auto_reorder_loop(), _daily_digest_loop(), _drift_check_loop(), _escalation_loop(), health(), _lifespan(), _predictive_loop(), Request (+45 more)

### Community 16 - "FileStorage"
Cohesion: 0.11
Nodes (23): GET from `url` with automatic retry on transient errors., resilient_get(), get_account_info(), get_broadcast_status(), get_conversation(), get_conversation_messages(), is_configured(), list_contacts() (+15 more)
...
Cohesion: 0.24
Nodes (3): _company_id_for_code(), Given a company domain/code, return its companies UUID., SupabaseMachineRepository
...
Cohesion: 0.12
Nodes (28): get_machines(), Return the configured MachineRepository implementation (cached singleton)., Background loop: fire fallback fan-outs for sessions that expired without notifi, _sweep_loop(), get_sessions(), Dependency that returns the module-level session store., Fire a fallback fan-out for any session that expired without being notified., sweep_expired_unnotified() (+20 more)
...
Cohesion: 0.18
Nodes (17): Report router — on-demand and scheduled maintenance reports., _compute_metrics(), _filter_tickets_in_range(), format_report_text(), generate_report(), _parse_dt(), _period_range(), _previous_period_range() (+9 more)
...
Cohesion: 0.10
Nodes (35): create_reset_token(), decode_access_token(), decode_reset_token(), get_current_admin(), get_current_user(), hash_password(), _password_fingerprint(), Phase 5 - Document Vault authentication.  A small, real (not stubbed) JWT auth l (+27 more)
...
Cohesion: 0.20
Nodes (14): get_technician_work(), get_tickets(), Return persistent technician work records using the active store backend., Return the configured TicketRepository implementation (cached singleton)., new_ticket_id(), _open_ticket(), _technician_token(), test_evidence_upload_is_persisted_and_downloadable() (+6 more)
...
Cohesion: 0.12
Nodes (36): FileStorage, ABC, Return file bytes for a storage_path previously returned by save()., Delete the file at storage_path. Silently ignores missing files., Save file bytes and return an opaque storage_path string for later retrieval., _approved_record_markdown(), build_machine_data(), _document_text() (+28 more)
...
Cohesion: 0.15
Nodes (5): LocalUserRepository, _normalize(), Local (openpyxl / Excel) implementation of UserRepository., Reads/writes Users and Companies tabs in the local tracker workbook., Look up a user by phone or email (case-insensitive, whitespace-trimmed).
...
Cohesion: 0.03
Nodes (90): Self-contained internal TurboFix platform administration console., create_admin_token(), Role, get_escalation_config(), CustomKpiRepository, ABC, Read/write access to the Users and Companies entities., Generate a new unique user ID scoped to a company. (+82 more)
...
Cohesion: 0.24
Nodes (14): admin_company_workspace_preview(), Return a safe, read-only representation of the customer's workspace., list_machines(), list_team(), reveal_team_contact(), can_reveal_contact(), company_hierarchy(), directory_entry() (+6 more)
...
Cohesion: 0.14
Nodes (6): get_part_requests(), _factory_id_for_code(), Given a company domain/code, return the corresponding factories UUID., GET rows. params are PostgREST query-string filters., SupabaseDocumentRepository, SupabasePartRequestRepository
...
Cohesion: 0.09
Nodes (20): new_kpi_entry_id(), new_kpi_id(), Abstract base classes (interfaces) for all TurboFix data repositories.  Every co, LocalCustomKpiRepository, In-memory implementation of CustomKpiRepository for local dev/testing., Google Sheets implementation of CustomKpiRepository., add_custom_kpi(), delete_custom_kpi() (+12 more)
...
Cohesion: 0.08
Nodes (28): Supabase (PostgREST) implementation of all TurboFix repositories.  Connects to t, SupabaseEscalationConfigRepository, get_technician_workload(), check_and_flag_on_creation(), check_repeat_failure(), confirm_ai_diagnosis(), flag_repeat_failure(), get_factory_ai_stats() (+20 more)
...
Cohesion: 0.10
Nodes (31): _graph_url(), WhatsApp messaging — routes through WaCRM when configured, else direct Meta Clou, Send a plain text message., Send turbofix_escalation in its six-placeholder order., Send a Meta template with body values in placeholder order., Send turbofix_ticket_closed: ticket, machine, resolution, closer, duration., send_approval_template(), send_closure_template() (+23 more)
...
## Knowledge Gaps
- **712 isolated node(s):** `Achievements`, `Time Investment`, `Priority 1: CRITICAL (1 bug)`, `Priority 2: HIGH (2 bugs)`, `Priority 3: MEDIUM-HIGH (1 bug)` (+707 more)
...
- **Why does `TicketRepository` connect `whatsapp_webhook/index.ts` to `imports`, `imports`, `UserRepository`, `main.py`, `TicketRepository`, `base.py`, `test_new_features.py`, `react`, `dependencies`, `TurboFix WhatsApp Brochure (Marketing Image)`, `whatsapp.py`, `test_webhook.py`, `escalation_service.py`, `LocalUserRepository`, `dependencies.py`, `get_tickets`, `report_service.py`, `LocalDocumentRepository`, `icons.svg (Icon Sprite Sheet)`, `get_technician_load`, `imports`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Why does `MachineRepository` connect `whatsapp_webhook/index.ts` to `imports`, `imports`, `UserRepository`, `main.py`, `TicketRepository`, `base.py`, `test_new_features.py`, `react`, `dependencies`, `TurboFix WhatsApp Brochure (Marketing Image)`, `whatsapp.py`, `test_webhook.py`, `escalation_service.py`, `LocalUserRepository`, `get_tickets`, `SheetsPartsRepository`, `report_service.py`, `LocalDocumentRepository`, `icons.svg (Icon Sprite Sheet)`, `logging.py`, `get_technician_load`, `imports`?**
