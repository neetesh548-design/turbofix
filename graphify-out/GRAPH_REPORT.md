...
## Corpus Check
- 343 files · ~422,552 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3420 nodes · 6475 edges · 259 communities (182 shown, 77 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 544 edges (avg confidence: 0.54)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `3af8e0d5`
...
Cohesion: 0.08
Nodes (24): CustomKpiRepository, AntDKPICard(), ClosedLoopControlCard(), asNumber(), buildMonthlyTrend(), computeBacklog(), computeBacklogVelocity(), computeCostRatios() (+16 more)
...
Cohesion: 0.09
Nodes (47): EventRepository, new_event_id(), Read/write access to the MachineEvents data entity., Local (openpyxl / Excel) implementations of TicketRepository and MachineReposito, ai_enabled(), Return True if any AI provider is configured and ready., _detect_and_store_language(), finish_audio_ticket() (+39 more)
...
### Community 24 - ".select"
Cohesion: 0.13
Nodes (26): Background loop: fire fallback fan-outs for sessions that expired without notifi, _sweep_loop(), get_sessions(), Dependency that returns the module-level session store., Fire a fallback fan-out for any session that expired without being notified., sweep_expired_unnotified(), _audio_payload(), _enable_fanout_credentials() (+18 more)
...
Cohesion: 0.16
Nodes (22): get_custom_kpis(), get_parts(), get_technician_work(), get_tickets(), get_users(), Return the configured UserRepository implementation (cached singleton)., Return the configured PartsRepository implementation (cached singleton)., Return the configured CustomKpiRepository implementation (cached singleton). (+14 more)
...
Cohesion: 0.08
Nodes (7): new_user_id(), LocalCustomKpiRepository, LocalUserRepository, _normalize(), Local (openpyxl / Excel) implementation of UserRepository., Reads/writes Users and Companies tabs in the local tracker workbook., Look up a user by phone or email (case-insensitive, whitespace-trimmed).
...
Cohesion: 0.24
Nodes (5): Closed-Loop Maintenance Lifecycle, TicketRepository, GATEWAY_I18N, QRGateway(), supabase
...
Cohesion: 0.38
Nodes (8): MachineRepository, generateChecklist(), similarity(), step(), STOP_WORDS, text(), tokens(), Machines()
...
Cohesion: 0.08
Nodes (19): react, Footer(), LanguageGate(), Navbar(), SkipLink(), LanguageContext, useLanguage(), MainLayout() (+11 more)
...
Cohesion: 0.09
Nodes (25): AdvancedFeaturesDrilldown(), AppShell(), getLiveDataAnswer(), isTokenExpired(), NAV_LIVE, NAV_SOON, readAuth(), ContactReveal() (+17 more)
...
### Community 253 - "AntDDashboardComponents.jsx"
Cohesion: 0.29
Nodes (3): AntDChartCard(), AntDDetailList(), AntDEmptyState()

### Community 255 - "Technician.jsx"
Cohesion: 0.50
...
## Knowledge Gaps
- **777 isolated node(s):** `defaultWork`, `1. Comprehensive Audit ✅`, `2. View Mode Toggle ✅`, `3. WhatsApp Integration Entry Point ✅`, `Production Readiness: 80% → 85%` (+772 more)
...
- **Why does `react` connect `LanguageContext.jsx` to `badge.jsx`, `button.jsx`, `whatsapp_webhook/index.ts`, `dashboardLayout.js`, `admin_router.py`, `TurboFix Brand Identity`, `summarize.py`, `gemini.py`, `PerformanceMonitor`, `test_vault_password_reset.py`, `dynamicChecklist.js`, `parse_message`, `test_supabase_machine_records.py`, `useI18n`, `LocalTechnicianWorkRepository`, `Tickets.jsx`, `AntDModalsAndFeedback.jsx`, `manifest.json`, `icons.svg (Icon Sprite Sheet)`, `i18n.js`, `LocalTechnicianWorkRepository`, `SupabaseCustomKpiRepository`, `get_technician_load`, `SheetsDocumentRepository`, `test_machine_records.py`, `card.jsx`, `StorageManager`, `ai_diagnostics/index.ts`, `SheetsTechnicianWorkRepository`, `AntDDashboardComponents.jsx`, `Technician.jsx`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Why does `TicketRepository` connect `UserRepository` to `CurrentUser`, `main.py`, `consumables_service.py`, `Dashboard.jsx`, `AppShell.jsx`, `Top Navigation Bar`, `.select`, `provider.py`, `logging.py`, `Records.jsx`, `auth_router.py`, `LocalUserRepository`, `SessionStore`, `scripts`, `SupabaseTicketRepository`, `test_vault_quota_admin.py`, `useI18n`, `ai_service.py`, `performance.jsx`, `_rate_limit_handler`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
