...
## Corpus Check
- 565 files · ~1,026,327 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4345 nodes · 7946 edges · 388 communities (260 shown, 128 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 355 edges (avg confidence: 0.63)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d61650fd`
...
Cohesion: 0.05
Nodes (56): get_file_storage(), Return the FileStorage implementation selected by DOCUMENT_STORE env var., DocumentRepository, MachineRecordRepository, PartsRepository, Read/write access to the Documents metadata entity., Generate a new unique document ID., Return all documents for a company (optionally filtered by machine). (+48 more)
...
Cohesion: 0.07
Nodes (28): Assistant, Dashboard, DemoLogin, Home, Inventory, Kaizen, Login, Machines (+20 more)
...
Cohesion: 0.12
Nodes (21): FileStorage, Return file bytes for a storage_path previously returned by save()., Delete the file at storage_path. Silently ignores missing files., Save file bytes and return an opaque storage_path string for later retrieval., _approved_record_markdown(), build_machine_data(), _document_text(), _internet_reference() (+13 more)

### Community 22 - "wacrm_client.py"
Cohesion: 0.02
Nodes (16): react, compressImage(), PhotoCapture(), TREND_COPY, FormInput(), URGENCY_TONE, iconBg, Notification() (+8 more)
...
Cohesion: 0.05
Nodes (48): ABC, MachineRepository, Read/write access to the Machines data entity., Return {machine_id: {...}} for all machines (may be cached)., Return the machine dict, or None if not found., Append a new machine row. Keys must match MACHINES_HEADER (minus has_open_ticket, Force the next load() to re-read from the backing store., Return the next Mnnn code for a company, e.g. 'M003'. (+40 more)
...
Cohesion: 0.14
Nodes (16): create_admin_token(), create_reset_token(), decode_access_token(), decode_reset_token(), get_current_admin(), hash_password(), _password_fingerprint(), Phase 5 - Document Vault authentication.  A small, real (not stubbed) JWT auth l (+8 more)
...
Cohesion: 0.13
Nodes (19): Any, _find_worksheet(), _get_all_values(), get_client(), get_spreadsheet(), get_worksheet(), _open_spreadsheet(), Shared, cached Google Sheets client for all Sheets-backed repositories.  A singl (+11 more)
...
Cohesion: 0.24
Nodes (13): create_access_token(), get_technician_work(), Return persistent technician work records using the active store backend., new_ticket_id(), _open_ticket(), _technician_token(), test_evidence_upload_is_persisted_and_downloadable(), test_submission_requires_complete_checklist_and_notes() (+5 more)
...
Cohesion: 0.08
Nodes (36): Read/write access to the Tickets data entity., Generate a new unique ticket ID., Set photo_media_id on the matching row. Returns True if found., Set the detected language on the matching ticket. Returns True if found., Mark a ticket as Closed with timestamp and who closed it. Returns True if found., Find a ticket whose ticket_id starts with prefix (case-insensitive)., TicketRepository, ai_enabled() (+28 more)
...
Cohesion: 0.12
Nodes (17): compute_kpis(), _machine_status_bucket(), Compute live KPI dashboard for a company. Pure calculation helper., Test count of machines with open tickets (machines_down)., Test count of urgent open tickets., Test total count of open tickets., Test plant health percentage (inverse of machines_down ratio)., Test calculation of average hours to fix (resolved tickets). (+9 more)
...
Cohesion: 0.08
Nodes (29): build_custom_kpi_values(), build_dashboard_overview(), compute_auto_insights(), get_dashboard_data(), get_machine_highlights(), get_tickets_by_assignee(), Dashboard service — compute per-company KPIs from live ticket/machine data., Evaluate custom KPI formula against repository metrics data. (+21 more)
...
Cohesion: 0.17
Nodes (15): CapabilityStrip(), ContactCard(), DEFAULT_FAQS, FaqAccordion(), PageHero(), ProofBanner(), contentByLanguage, faqs (+7 more)
...
Cohesion: 0.11
Nodes (40): Return an error message if the password is too weak, else None., Role, validate_password_strength(), verify_password(), add_supervisor(), AddSupervisorRequest, _company_approved(), _extract_10_digit_phone() (+32 more)
...
Cohesion: 0.16
Nodes (18): get_sessions(), _iter_messages(), Webhook router — WhatsApp webhook receive + verify.  This router is intentionall, Verify X-Wacrm-Signature: t=unix,v1=hmac-sha256., Receive webhook events from WaCRM.      WaCRM fires events for: message.received, Verify the X-Hub-Signature-256 HMAC from Meta., Dependency that returns the module-level session store., Receive and dispatch an incoming WhatsApp message.      Returns 200 immediately (+10 more)

### Community 266 - "email_client.py"
Cohesion: 0.14
Nodes (14): initialize_ticket_escalation(), Set the first escalation timer on a newly created ticket., handle_issue_part_command(), handle_parts_request_command(), handle_po_approve_command(), handle_po_reject_command(), handle_text_message(), Handle an incoming text message — escalation commands, closure, or new ticket. (+6 more)
...
Cohesion: 0.14
Nodes (17): escalate_part_request(), escalate_ticket(), _minutes_open(), Escalation engine — checks ticket timers and fires WhatsApp notifications.  Two, Advance a part request to the next escalation level., Technician submits evidence photo; notify Maintenance Head for approval., Maintenance Head rejects closure — ticket reopened, technician notified., Check all open tickets and part requests for overdue escalations. (+9 more)
...
Cohesion: 0.10
Nodes (26): get_custom_kpis(), get_documents(), get_events(), get_machine_records(), get_machines(), get_parts(), get_settings(), get_tickets() (+18 more)
...
Cohesion: 0.20
Nodes (3): LocalPartsRepository, Local (openpyxl / Excel) implementation of PartsRepository., Reads/writes spare parts and consumables in the local workbook.
...
Cohesion: 0.09
Nodes (26): EventRepository, new_event_id(), Read/write access to the MachineEvents data entity., Append a new event row. Keys must match MACHINE_EVENTS_HEADER., Return all events for a machine, oldest first., Return all events for a company., approve_ticket_closure(), delegate_to_colleague() (+18 more)
...
### Community 381 - ".append"
Cohesion: 0.22
Nodes (4): Footer(), useLanguage(), Contact(), WhyTurboFix()
...
## Knowledge Gaps
- **804 isolated node(s):** `Home`, `WhyTurboFix`, `Platform`, `RecordsPlatform`, `Workflow` (+799 more)
