...
## Corpus Check
- 560 files · ~762,712 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4338 nodes · 8059 edges · 366 communities (234 shown, 132 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 357 edges (avg confidence: 0.63)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `150b9c94`
...
## Communities (366 total, 132 thin omitted)
...
Cohesion: 0.08
Nodes (16): Read/write access to the Tickets data entity., Generate a new unique ticket ID., Append a new ticket row. Keys must match TICKETS_HEADER., Return the ticket dict for ticket_id, or None if not found., Set voice_note_media_id on the matching row. Returns True if found., Update AI-generated fields on the matching ticket. Returns True if found., Return all tickets belonging to a company., Set photo_media_id on the matching row. Returns True if found. (+8 more)
...
Cohesion: 0.03
Nodes (9): react, TREND_COPY, FormInput(), iconBg, STAGE_ACTIONS, Button(), buttonVariants, EXPLORE_LINKS (+1 more)
...
Cohesion: 0.22
Nodes (16): ai_enabled(), Return True if any AI provider is configured and ready., read_machine_data(), _detect_and_store_language(), finish_audio_ticket(), finish_image_ticket(), finish_text_ticket(), _merge_description() (+8 more)
...
Cohesion: 0.14
Nodes (12): EventRepository, Read/write access to the MachineEvents data entity., Append a new event row. Keys must match MACHINE_EVENTS_HEADER., Return all events for a machine, oldest first., Return all events for a company., handle_approve_command(), handle_audio_message(), handle_image_message() (+4 more)
...
Cohesion: 0.12
Nodes (16): handle_confirm_ai_command(), handle_issue_part_command(), handle_override_ai_command(), handle_parts_request_command(), handle_po_approve_command(), handle_po_reject_command(), handle_reject_command(), handle_text_message() (+8 more)
...
Cohesion: 0.06
Nodes (44): build_custom_kpi_values(), build_dashboard_overview(), compute_kpis(), get_dashboard_data(), get_machine_highlights(), get_tickets_by_assignee(), _machine_status_bucket(), Dashboard service — compute per-company KPIs from live ticket/machine data. (+36 more)
...
Cohesion: 0.19
Nodes (13): new_event_id(), handle_close_command(), handle_delegate_command(), handle_outsource_command(), handle_waiting_parts_command(), _log_event(), _phone_authorized_for_ticket(), Log an event to the MachineEvents tab. (+5 more)
...
Cohesion: 0.07
Nodes (28): download_media(), _graph_url(), WhatsApp messaging — routes through WaCRM when configured, else direct Meta Clou, Send turbofix_new_ticket: ticket, machine, location, issue, urgency, assignee., Send turbofix_ticket_closed: ticket, machine, resolution, closer, duration., Send a plain text message., Send turbofix_escalation in its six-placeholder order., Send a broadcast to multiple recipients. Only available via WaCRM.      recipien (+20 more)
...
Cohesion: 0.07
Nodes (29): initialize_part_request_escalation(), initialize_ticket_escalation(), Set the first escalation timer on a newly created ticket., Set the first escalation timer on a new part request., Integration tests for escalation_service.py  Tests ticket escalation workflow: -, Test supervisor rejecting technician's closure (incomplete work)., Test technician delegating ticket to colleague., Test marking issue as outsourced (vendor/contractor involvement). (+21 more)
...
## Knowledge Gaps
- **800 isolated node(s):** `Home`, `WhyTurboFix`, `Platform`, `RecordsPlatform`, `Workflow` (+795 more)
...
- **What is the exact relationship between `Bluesky Icon (butterfly logo)` and `Social/Community Icon (person with badge/ribbon, purple stroke)`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `react` connect `wacrm_client.py` to `machine_record_service.py`, `auth.test.js`, `Support.jsx`, `FakeAuthClient`, `LocalTicketRepository`, `handle_parts_request_command`, `MachineRepository`, `machineIdOf`, `kpi_router.py`, `config.py`, `Top Navigation Bar`, `test_vault_auth.py`, `PhotoCapture.jsx`, `ticketQueues.js`, `intelligence_service.py`, `SupabaseCustomKpiRepository`, `conftest.py`, `gemini.py`, `health`, `test_report_router.py`, `PerformanceMonitor`, `dynamicChecklist.js`, `SheetsCustomKpiRepository`, `machine_data_service.py`, `test_report_router.py`, `LocalMachineRecordRepository`, `LocalTechnicianWorkRepository`, `SupabaseMachineRecordRepository`, `Kaizen.jsx`, `FakeAsyncClient`, `.oxlintrc.json`, `MachineDetailDrawer.jsx`, `.list_data`, `tabs.jsx`, `roles.js`, `.next_ticket_id`, `Dashboard.jsx`, `MachinesRefactored.jsx`?**
  _High betweenness centrality (0.059) - this node is a cross-community bridge._
- **Why does `SupabaseTicketRepository` connect `LocalUserRepository` to `test_supabase_machine_records.py`, `MachinesRefactored.jsx`, `_sheets_unavailable_handler`, `SheetsPartsRepository`, `useI18n`, `SupervisorKaizen.jsx`, `ticket_gateway/index.ts`?**
  _High betweenness centrality (0.013) - this node is a cross-community bridge._
- **Why does `react` connect `LocalTicketRepository` to `Kaizen.jsx`, `handle_parts_request_command`, `SupabaseCustomKpiRepository`, `MachinesRefactored.jsx`, `config.py`, `AppShell.jsx`, `wacrm_client.py`, `LocalTechnicianWorkRepository`?**
