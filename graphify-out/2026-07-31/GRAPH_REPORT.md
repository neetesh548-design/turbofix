...
## Corpus Check
- 560 files · ~995,255 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4338 nodes · 8042 edges · 383 communities (247 shown, 136 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 356 edges (avg confidence: 0.63)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4e151e74`
...
### Community 9 - "main.py"
Cohesion: 0.05
Nodes (44): Sends a downloaded voice note to OpenAI's transcription API and returns the, transcribe_audio(), _is_retryable(), Resilient HTTP client with tenacity retry + exponential backoff.  Wraps httpx fo, configure_logging(), get_logger(), Structured JSON logging for TurboFix, backed by structlog.  Every log event prod, Call once at application startup (from main.py lifespan). (+36 more)
...
Cohesion: 0.03
Nodes (10): react, compressImage(), PhotoCapture(), CONFIG, TREND_COPY, FormInput(), Notification(), TYPE_STYLES (+2 more)
...
Cohesion: 0.15
Nodes (21): _graph_url(), WhatsApp messaging — routes through WaCRM when configured, else direct Meta Clou, Send turbofix_ticket_closed: ticket, machine, resolution, closer, duration., Send a plain text message., Send turbofix_escalation in its six-placeholder order., Send a broadcast to multiple recipients. Only available via WaCRM.      recipien, Send a Meta template with body values in placeholder order., send_approval_template() (+13 more)

### Community 46 - "conftest.py"
Cohesion: 0.07
Nodes (43): AppShell(), getLiveDataAnswer(), AuditLog(), MOCK_LOGS, Assistant(), getLiveDataAnswer(), machineSuggestions, plantSuggestions (+35 more)
...
Cohesion: 0.10
Nodes (36): create_access_token(), get_custom_kpis(), get_documents(), get_events(), get_machine_records(), get_machines(), get_parts(), get_settings() (+28 more)
...
### Community 291 - "PhotoCapture.jsx"
Cohesion: 0.25
...
Cohesion: 0.09
Nodes (12): get_part_requests(), _expand_encoded_json(), MachineRecordRepository, Supabase TEXT has no Excel cell limit, so persist readable JSON., Durable, tenant-scoped AI machine records stored in Supabase., Thin wrapper around httpx for PostgREST calls., GET rows. params are PostgREST query-string filters., _SupabaseClient (+4 more)
...
## Knowledge Gaps
- **800 isolated node(s):** `ROLE_HEADINGS`, `ROLE_HEADINGS`, `STATUS_FILTERS`, `EMPTY_FILTERS`, `ROLE_HEADINGS` (+795 more)
...
- **What is the exact relationship between `Bluesky Icon (butterfly logo)` and `Social/Community Icon (person with badge/ribbon, purple stroke)`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `react` connect `wacrm_client.py` to `machine_record_service.py`, `auth.test.js`, `Support.jsx`, `FakeAuthClient`, `LocalTicketRepository`, `MachineRepository`, `machineIdOf`, `kpi_router.py`, `config.py`, `Top Navigation Bar`, `test_vault_auth.py`, `ShutdownPlanner.jsx`, `Inventory.jsx`, `ticketQueues.js`, `intelligence_service.py`, `SupabaseCustomKpiRepository`, `conftest.py`, `gemini.py`, `health`, `test_report_router.py`, `PerformanceMonitor`, `dynamicChecklist.js`, `SheetsCustomKpiRepository`, `machine_data_service.py`, `RCA.jsx`, `test_report_router.py`, `LocalMachineRecordRepository`, `LocalTechnicianWorkRepository`, `Dashboard.jsx`, `Kaizen.jsx`, `FakeAsyncClient`, `.oxlintrc.json`, `StitchVisualCharts.jsx`, `Kaizen.jsx`, `MachineDetailDrawer.jsx`, `.list_data`, `tabs.jsx`, `roles.js`, `.next_ticket_id`, `Technician.jsx`, `Dashboard.jsx`, `Navbar.jsx`, `MachinesRefactored.jsx`, `ViewModeContext.jsx`, `MicrosoftAppLauncher.jsx`, `TicketDetailPanel.jsx`, `Home.jsx`?**
  _High betweenness centrality (0.047) - this node is a cross-community bridge._
- **Why does `MachineRepository` connect `devDependencies` to `SupabaseCustomKpiRepository`, `whatsapp_webhook/index.ts`, `DocumentRepository`, `PhotoCapture.jsx`, `fanout_service.py`, `webhook_router.py`, `handle_approve_command`, `BaseModel`, `MachinesRefactored.jsx`, `handle_delegate_command`, `SheetsTicketRepository`, `SupabasePartsRepository`, `SupabaseTicketRepository`, `dependencies`, `SupervisorKaizen.jsx`, `useI18n`?**
  _High betweenness centrality (0.012) - this node is a cross-community bridge._
- **Why does `TicketRepository` connect `fanout_service.py` to `SupabaseCustomKpiRepository`, `whatsapp_webhook/index.ts`, `DocumentRepository`, `webhook_router.py`, `handle_approve_command`, `BaseModel`, `MachinesRefactored.jsx`, `handle_delegate_command`, `SupabaseTicketRepository`, `useI18n`, `SupervisorKaizen.jsx`, `devDependencies`?**
