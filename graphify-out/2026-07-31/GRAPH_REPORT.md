...
## Corpus Check
- 560 files · ~762,687 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4337 nodes · 8058 edges · 372 communities (248 shown, 124 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 357 edges (avg confidence: 0.63)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e05cc521`
...
## Communities (372 total, 124 thin omitted)
...
Cohesion: 0.03
Nodes (9): react, TREND_COPY, FormInput(), Notification(), TYPE_STYLES, STAGE_ACTIONS, Button(), buttonVariants (+1 more)
...
Cohesion: 0.05
Nodes (51): ABC, get_current_user(), get_machines(), Return the configured MachineRepository implementation (cached singleton)., MachineRepository, Read/write access to the Machines data entity., Return {machine_id: {...}} for all machines (may be cached)., Return the machine dict, or None if not found. (+43 more)
...
Cohesion: 0.24
Nodes (4): _company_id_for_code(), Given a company domain/code, return its companies UUID., Thin wrapper around httpx for PostgREST calls., _SupabaseClient
...
### Community 255 - "_text_payload"
Cohesion: 0.25
Nodes (4): _events_for_machine(), TestMachineEvents, TestTicketClosure, _text_payload()
...
Cohesion: 0.16
Nodes (25): CapabilityStrip(), ContactCard(), DEFAULT_FAQS, FaqAccordion(), PageHero(), PricingCalculator(), ProofBanner(), contentByLanguage (+17 more)
...
Cohesion: 0.12
Nodes (7): _expand_encoded_json(), CustomKpiRepository, TechnicianWorkRepository, Supabase (PostgREST) implementation of all TurboFix repositories.  Connects to t, Supabase TEXT has no Excel cell limit, so persist readable JSON., SupabaseCustomKpiRepository, SupabaseTechnicianWorkRepository
...
Cohesion: 0.09
Nodes (32): get_custom_kpis(), get_documents(), get_escalation_config(), get_events(), get_machine_records(), get_parts(), get_settings(), get_technician_work() (+24 more)
...
Cohesion: 0.12
Nodes (28): Read/write access to company-scoped operational settings., Return the settings row for a company, or None when not configured., Create or replace the settings row for one company., SettingsRepository, add_custom_role(), _company_approved(), _company_quota(), create_machine() (+20 more)
...
Cohesion: 0.07
Nodes (8): new_item_id(), Abstract base classes (interfaces) for all TurboFix data repositories.  Every co, LocalPartsRepository, Local (openpyxl / Excel) implementation of PartsRepository., Reads/writes spare parts and consumables in the local workbook., LocalTechnicianWorkRepository, Excel-backed technician work records., Local (openpyxl / Excel) implementation of UserRepository.
...
Cohesion: 0.10
Nodes (10): get_part_requests(), _build_factory_to_code_map(), MachineRecordRepository, Durable, tenant-scoped AI machine records stored in Supabase., Build a mapping of factory_id -> company_code in bulk with minimal queries., GET rows. params are PostgREST query-string filters., SupabaseEventRepository, SupabaseMachineRecordRepository (+2 more)

### Community 355 - "test_machine_records.py"
Cohesion: 0.23
Nodes (12): create_access_token(), new_ticket_id(), test_backup_contains_originals_structured_data_csv_and_machine_data(), test_duplicate_source_is_rejected(), test_non_owner_operational_role_can_create_review_draft(), test_only_maintenance_head_can_approve_and_approved_data_reaches_machine_context(), test_restore_requires_maintenance_head(), test_supervisor_can_upload_and_company_isolation_is_enforced() (+4 more)
...
## Knowledge Gaps
- **799 isolated node(s):** `Home`, `WhyTurboFix`, `Platform`, `RecordsPlatform`, `Workflow` (+794 more)
...
- **What is the exact relationship between `Bluesky Icon (butterfly logo)` and `Social/Community Icon (person with badge/ribbon, purple stroke)`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `react` connect `wacrm_client.py` to `machine_record_service.py`, `auth.test.js`, `Support.jsx`, `FakeAuthClient`, `LocalTicketRepository`, `MachineRepository`, `machineIdOf`, `kpi_router.py`, `config.py`, `Top Navigation Bar`, `test_vault_auth.py`, `PhotoCapture.jsx`, `ticketQueues.js`, `intelligence_service.py`, `SupabaseCustomKpiRepository`, `conftest.py`, `gemini.py`, `health`, `test_report_router.py`, `MicrosoftAppLauncher.jsx`, `PerformanceMonitor`, `dynamicChecklist.js`, `SheetsCustomKpiRepository`, `machine_data_service.py`, `Home.jsx`, `test_report_router.py`, `LocalMachineRecordRepository`, `LocalTechnicianWorkRepository`, `SupabaseMachineRecordRepository`, `Kaizen.jsx`, `FakeAsyncClient`, `.oxlintrc.json`, `test_admin_onboarding.py`, `MachineDetailDrawer.jsx`, `.list_data`, `tabs.jsx`, `roles.js`, `.next_ticket_id`, `Dashboard.jsx`, `MachinesRefactored.jsx`?**
  _High betweenness centrality (0.061) - this node is a cross-community bridge._
- **Why does `TicketRepository` connect `fanout_service.py` to `SupabaseCustomKpiRepository`, `whatsapp_webhook/index.ts`, `DocumentRepository`, `webhook_router.py`, `BaseModel`, `scripts`, `MachinesRefactored.jsx`, `handle_delegate_command`, `SupabaseTicketRepository`, `SupabaseTechnicianWorkRepository`, `SupervisorKaizen.jsx`, `useI18n`, `_rate_limit_handler`, `devDependencies`?**
  _High betweenness centrality (0.014) - this node is a cross-community bridge._
- **Why does `MachineRepository` connect `devDependencies` to `SupabaseCustomKpiRepository`, `whatsapp_webhook/index.ts`, `DocumentRepository`, `fanout_service.py`, `handle_parts_request_command`, `webhook_router.py`, `BaseModel`, `scripts`, `MachinesRefactored.jsx`, `handle_delegate_command`, `SheetsTicketRepository`, `SupabaseTicketRepository`, `SupabaseTechnicianWorkRepository`, `SupervisorKaizen.jsx`, `_rate_limit_handler`, `dependencies`, `useI18n`?**
