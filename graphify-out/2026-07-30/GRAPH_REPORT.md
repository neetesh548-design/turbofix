...
## Corpus Check
- 540 files · ~645,871 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4242 nodes · 8039 edges · 375 communities (241 shown, 134 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 371 edges (avg confidence: 0.64)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `ed57f30a`
...
## Communities (375 total, 134 thin omitted)
...
Cohesion: 0.04
Nodes (67): ABC, MachineRepository, new_document_id(), Read/write access to the Tickets data entity., Generate a new unique ticket ID., Append a new ticket row. Keys must match TICKETS_HEADER., Return the ticket dict for ticket_id, or None if not found., Set voice_note_media_id on the matching row. Returns True if found. (+59 more)
...
Cohesion: 0.08
Nodes (6): new_event_id(), LocalTicketRepository, Local (openpyxl / Excel) implementations of TicketRepository and MachineReposito, Reads/writes tickets in the Tickets tab of the local tracker workbook., Tests for the local (openpyxl) ticket and machine repositories.  Replaces test_s, ticket_repo()

### Community 11 - "base.py"
Cohesion: 0.11
Nodes (17): Integration tests for dashboard_service.py  Tests KPI computation and dashboard, Sample ticket data for dashboard tests., Test that dashboard is isolated per company., Test calculation of a single custom KPI., Test KPI with multiple field references., Test KPI calculation handles division by zero gracefully., Test that computed KPIs are cached for 5 minutes., Test dashboard performance with 1000+ tickets. (+9 more)
...
Cohesion: 0.07
Nodes (28): App(), Assistant, Dashboard, Home, Inventory, Kaizen, Login, Machines (+20 more)
...
Cohesion: 0.04
Nodes (8): react, FormInput(), Notification(), TYPE_STYLES, STAGE_ACTIONS, Badge(), badgeVariants, MachineContext

### Community 23 - "Top Navigation Bar"
Cohesion: 0.22
Nodes (5): Read canonical records while tolerating old, extra, or blank columns.      Produ, read_records(), _normalize(), Reads/writes Users and Companies worksheets in a Google Sheet., SheetsUserRepository
...
Cohesion: 0.14
Nodes (20): admin_company_dashboard(), get_sessions(), _iter_messages(), Webhook router — WhatsApp webhook receive + verify.  This router is intentionall, Verify X-Wacrm-Signature: t=unix,v1=hmac-sha256., Receive webhook events from WaCRM.      WaCRM fires events for: message.received, Verify the X-Hub-Signature-256 HMAC from Meta., Dependency that returns the module-level session store. (+12 more)
...
Cohesion: 0.08
Nodes (31): Any, _get_all_values(), dispatch_daily_digest(), dispatch_weekly_digest(), format_daily_digest_message(), format_weekly_digest_message(), generate_daily_digest_payload(), generate_weekly_digest_payload() (+23 more)
...
Cohesion: 0.27
Nodes (7): IssueCapture(), QUICK_PHRASES, PhotoAnnotatorModal(), QuickReportDialog(), URGENCY_ORDER, urgencyMeta(), microphoneErrorMessage()

### Community 48 - ".get"
Cohesion: 0.09
Nodes (31): _content_matches_extension(), FileStorage, FileTooLargeError, ABC, Pluggable file storage — local disk (dev/test) and Google Drive (production).  T, Return file bytes for a storage_path previously returned by save()., Delete the file at storage_path. Silently ignores missing files., Check that file content magic bytes match the claimed extension. (+23 more)
...
Cohesion: 0.07
Nodes (42): get_custom_kpis(), get_documents(), get_events(), get_machine_records(), get_machines(), get_parts(), get_settings(), get_technician_work() (+34 more)
...
Cohesion: 0.13
Nodes (24): _graph_url(), WhatsApp messaging — routes through WaCRM when configured, else direct Meta Clou, Send turbofix_new_ticket: ticket, machine, location, issue, urgency, assignee., Send turbofix_ticket_closed: ticket, machine, resolution, closer, duration., Send a plain text message., Send turbofix_escalation in its six-placeholder order., Send a broadcast to multiple recipients. Only available via WaCRM.      recipien, Send a Meta template with body values in placeholder order. (+16 more)
...
Cohesion: 0.18
Nodes (11): AuditLog(), MOCK_LOGS, FISHBONE_CATEGORIES, INITIAL_WHYS, RCA(), readParam(), readStoredUser(), suggestKaizen() (+3 more)

### Community 98 - "test_admin_onboarding.py"
Cohesion: 0.14
Nodes (9): TechnicianWorkRepository, AdvancedFeaturesDrilldown(), AppShell(), getLiveDataAnswer(), ContactReveal(), EmptyState(), AccordionSections(), defaultWork (+1 more)
...
Cohesion: 0.30
Nodes (12): MachinesRefactored(), filterRowsToVisibleMachines(), isMachineAssignedToTechnician(), isMachineVisibleToShiftUser(), isShiftScopedRole(), isTechnicianRole(), machineId(), SHIFT_SCOPED_ROLES (+4 more)
...
Cohesion: 0.11
Nodes (22): _auto_reorder_loop(), _daily_digest_loop(), _drift_check_loop(), _escalation_loop(), health(), _lifespan(), _predictive_loop(), TurboFix backend entry point — thin application bootstrap.  This module's only j (+14 more)
...
Cohesion: 0.13
Nodes (9): Sends a downloaded voice note to OpenAI's transcription API and returns the, transcribe_audio(), _cell_value(), main(), Creates the live Google Sheet ("TurboFix-Tracker-Live") from TurboFix-Tracker.xl, FakeAsyncClient, FakeResponse, test_transcribe_audio_returns_stripped_text() (+1 more)
...
Cohesion: 0.14
Nodes (26): assignedEngineer(), assignedSupervisor(), buildBreakdownRecord(), CATEGORY_INDEX, classifyIssue(), isAtLeastUrgent(), ISSUE_CATEGORIES, KEYWORD_PATTERNS (+18 more)
...
Cohesion: 0.12
Nodes (10): DEMO_BREAKDOWN_MACHINES, buildDemoTickets(), daysAgo(), DEMO_MACHINES, DEMO_PM_LOGS, DEMO_TEAM, hoursAgo(), DEMO_MACHINES (+2 more)
...
Cohesion: 0.12
Nodes (6): download_media(), Resolve a WhatsApp media ID → download → save to MEDIA_STORE_DIR.      Returns t, FakeAsyncClient, FakePostAsyncClient, FakeResponse, test_download_media_saves_file()
...
### Community 333 - "dashboard_service.py"
Cohesion: 0.31
...
### Community 341 - "machine_data_service.py"
Cohesion: 0.33
Nodes (6): RoleForm(), BREAKDOWN_ROLES, hourlyDowntimeCost(), NEXT_ACTIONS, REPORT_REASONS, TECH_SEVERITY_OPTIONS

### Community 345 - "get_technician_load"
Cohesion: 0.09
Nodes (26): check_and_flag_on_creation(), confirm_ai_diagnosis(), extract_machine_record(), flag_repeat_failure(), get_factory_ai_stats(), get_least_loaded_technician(), get_machine_ai_accuracy(), get_shift_config() (+18 more)

### Community 353 - "test_sheets_user_repo.py"
Cohesion: 0.47
...
### Community 356 - "LanguageContext.jsx"
Cohesion: 0.60
...
## Knowledge Gaps
- **786 isolated node(s):** `Home`, `WhyTurboFix`, `Platform`, `RecordsPlatform`, `Workflow` (+781 more)
...
- **What is the exact relationship between `Bluesky Icon (butterfly logo)` and `Social/Community Icon (person with badge/ribbon, purple stroke)`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `react` connect `wacrm_client.py` to `machine_record_service.py`, `Support.jsx`, `FakeAuthClient`, `LocalTicketRepository`, `QRGateway.jsx`, `MachineRepository`, `kpi_router.py`, `config.py`, `test_sheets_user_repo.py`, `SheetsTicketRepository`, `test_sheets_user_repo.py`, `TurboFix WhatsApp Brochure (Marketing Image)`, `machineExport.js`, `MainLayout.jsx`, `test_contact_privacy.py`, `ticketQueues.js`, `intelligence_service.py`, `SupabaseCustomKpiRepository`, `gemini.py`, `OperatorKaizen.jsx`, `PerformanceMonitor`, `dynamicChecklist.js`, `SheetsCustomKpiRepository`, `machine_data_service.py`, `SupabasePartsRepository`, `machine_data_service.py`, `check_and_flag_on_creation`, `LocalMachineRecordRepository`, `LocalTechnicianWorkRepository`, `SupabaseMachineRecordRepository`, `test_admin_onboarding.py`, `FakeAsyncClient`, `.oxlintrc.json`, `Kaizen.jsx`, `LanguageContext.jsx`, `PhotoCapture.jsx`, `dashboardLayout.js`, `SheetsTechnicianWorkRepository`, `tabs.jsx`, `roles.js`, `Tickets.jsx`, `NotificationCenter.jsx`, `ViewModeContext.jsx`, `Dashboard.jsx`, `MachinesRefactored.jsx`, `dashboardLayout.js`?**
  _High betweenness centrality (0.040) - this node is a cross-community bridge._
- **Why does `TicketRepository` connect `fanout_service.py` to `DocumentRepository`, `TicketRepository`, `.insert`, `resilient_get`, `conftest.py`, `LocalDocumentRepository`, `SupabaseTicketRepository`, `I18nManager`, `whatsapp.py`, `test_webhook.py`?**
  _High betweenness centrality (0.024) - this node is a cross-community bridge._
- **Why does `TechnicianWorkRepository` connect `fanout_service.py` to `SupabaseEscalationConfigRepository`, `I18nManager`, `whatsapp.py`?**
