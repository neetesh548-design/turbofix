...
Given a description (typed or transcribed from a voice note), the backend calls an AI
provider to produce a structured brief and writes it into the `ai_summary` and
`urgency` columns. Two interchangeable providers (`app/ai/provider.py` dispatches):

- **Gemini — the free option, default for demos/pilots** (`app/ai/gemini.py`) — one
  free-tier API key (from https://aistudio.google.com/apikey, no billing account
  needed) covers both jobs: voice notes are sent inline to `generateContent` for
  transcription (Gemini understands audio natively), and summarization uses the same
  endpoint with JSON output mode. Model set by `GEMINI_MODEL` (default
  `gemini-2.5-flash`). Free-tier rate limits are far above demo traffic.
- **OpenAI — the paid path** (`app/ai/transcribe.py` + `app/ai/summarize.py`) — the
  audio transcription endpoint (`OPENAI_TRANSCRIBE_MODEL`, default
  `gpt-4o-mini-transcribe`) and a JSON chat completion (`OPENAI_CHAT_MODEL`, default
  `gpt-4.1-nano`), per the cost model in `../progress.md`.

`AI_PROVIDER` picks: `auto` (default) uses Gemini if `GEMINI_API_KEY` is set, else
OpenAI if `OPENAI_API_KEY` is set, else the AI layer is off. Explicitly setting
`AI_PROVIDER=gemini`/`openai` without that provider's key turns the layer off rather
than silently falling through to the other (possibly paid) provider.

Both providers share the same prompt, urgency normalization, and `IssueBrief` output
contract, so everything downstream (Sheet columns, fan-out message, dashboard) is
identical regardless of provider.

**Graceful degradation is load-bearing here, not optional:** if no AI key is set, or
any AI call fails for any reason, the ticket still gets logged — it just keeps blank
`ai_summary`/`urgency` (and, for a voice note, the raw `media_id` without a
...
To also see the AI layer run, set `GEMINI_API_KEY` (free — see Phase 2 above) or
`OPENAI_API_KEY` in `.env` before starting the server — with a real key, the ticket
above will get `ai_summary`/`urgency` filled in within a few seconds of the request.
...
`tmp_path` first. WhatsApp media downloads and all AI calls (Gemini and OpenAI,
transcription and summarization) are mocked in tests — no real network calls, no API
...
To turn on the AI layer for free (demo/pilot), get a Gemini API key at
https://aistudio.google.com/apikey and set `GEMINI_API_KEY` in `.env` — done, both
transcription and summarization work at zero cost. For the paid path, set
`OPENAI_API_KEY` instead — `OPENAI_TRANSCRIBE_MODEL`/`OPENAI_CHAT_MODEL` default to
the models already priced out in `../progress.md`'s cost model, so leave them unless
you're deliberately changing the cost basis.
