...
def _content_matches_extension(ext: str, content: bytes) -> bool:
    """Check that file content magic bytes match the claimed extension."""
...
def validate_upload(filename: str, size_bytes: int, content: bytes | None = None) -> None:
    ...
    if content is not None and not _content_matches_extension(ext, content):
        raise UnsupportedFileTypeError(
