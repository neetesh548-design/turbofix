  ...
  return <div className="records-modal" role="dialog" aria-modal="true" aria-labelledby="upload-records-title">
    ...
    <form className="records-upload-dialog" onSubmit={submit}>
      ...
      <div className="records-form-grid">
        <label><span>Machine</span><select value={machineId} onChange={(event) => setMachineId(event.target.value)} required autoFocus>{machines.map((machine) => <option key={machine.machine_id} value={machine.machine_id}>{machine.machine_name} · {machine.location || machine.machine_id}</option>)}</select></label>
    ...
    <div className="records-review-dialog">
      ...
      <div className="records-review-layout">
        ...
        <main className="records-editor">
          ...
          <section className="records-edit-section"><div className="records-edit-heading"><div><span>01</span><h3>Record summary</h3></div></div><textarea className="records-summary-input" value={draft.summary || ''} disabled={locked} autoFocus={!locked} onChange={(event) => setDraft((current) => ({ ...current, summary: event.target.value }))} placeholder="Short explanation of what this record contains" /></section>
...
export default function Records() {
  ...
  return <AppShell active="records">
    <div className="records-page" data-testid="records-page">
      ...
      {tab === 'inbox' && <section className="records-panel"><div className="records-panel-head"><div><h2>Recent activity</h2><p>Last 5 critical changes</p></div><span className="records-head-note"><ShieldCheck />Maintenance Head approval required</span></div>{loading ? <div className="records-empty"><Sparkles className="spin" /><strong>Loading machine records…</strong></div> : filtered.length === 0 ? <div className="records-empty"><FileSearch /><strong>{records.length ? 'No records uploaded yet' : 'No records uploaded yet'}</strong><span>{records.length ? 'Clear a filter or try another machine.' : 'Start with a service register, job card, manual, BOM, or inspection sheet.'}</span>{!records.length && <button className="records-button primary" onClick={() => setUploadOpen(true)}><Plus />Add first record</button>}</div> : <><div className="records-table"><div className="records-table-head"><span>Record</span><span>Machine</span><span>Status</span><span>Updated</span><span /></div>{filtered.slice(0, 5).map((record) => <button className="records-table-row" key={record.record_id} onClick={() => setSelectedRecord(record)}><span className="records-title-cell"><i>{record.source_kind === 'handwritten' ? <Image /> : <FileText />}</i><span><strong>{record.title}</strong><small>{record.file_name || 'Structured record'}</small></span></span><span><strong>{machineMap[record.machine_id]?.machine_name || record.machine_id}</strong></span><StatusBadge status={record.status} /><span><strong>{formatDate(record.updated_at)}</strong></span><ChevronRight /></button>)}</div><button type="button" className="records-button secondary full" onClick={() => setShowMoreOptions(!showMoreOptions)} style={{ marginTop: '16px' }}>More options</button>{showMoreOptions && <><div style={{ marginTop: '24px', paddingTop: '24px', borderTop: '1px solid rgba(255,255,255,0.1)' }}><div className="records-filters"><label className="records-search"><Search /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search records or machines" /></label><select value={machineFilter} onChange={(event) => setMachineFilter(event.target.value)}><option value="all">All machines</option>{machines.map((machine) => <option value={machine.machine_id} key={machine.machine_id}>{machine.machine_name}</option>)}</select><select value={statusFilter} onChange={(event) => { setStatusFilter(event.target.value); setLowConfidenceOnly(false); }}><option value="all">All statuses</option><option value="needs_review">Needs review</option><option value="approved">Approved</option><option value="rejected">Returned</option></select></div>{lowConfidenceOnly && <button type="button" className="records-active-filter" onClick={() => setLowConfidenceOnly(false)}>Low confidence only <X /></button>}{filtered.map((record) => <button className="records-table-row" key={record.record_id} onClick={() => setSelectedRecord(record)}><span className="records-title-cell"><i>{record.source_kind === 'handwritten' ? <Image /> : <FileText />}</i><span><strong>{record.title}</strong><small>{label(record.record_type)} · {record.file_name || 'Structured record'}</small></span></span><span><strong>{machineMap[record.machine_id]?.machine_name || record.machine_id}</strong><small>{machineMap[record.machine_id]?.location || record.machine_id}</small></span><Confidence value={record.overall_confidence} /><StatusBadge status={record.status} /><span><strong>{formatDate(record.updated_at)}</strong><small>Version {record.version}</small></span><ChevronRight /></button>)}</div></>}</>}</section>}
